...
/// in general that *do* require a fixed key length. `hmac_sha256_hex` below
/// already documents that same fact for the identical reason and still
/// chooses not to `expect` it away; this crate does not carry a second
/// `expect` resting on it. [`block_sized_key`] does only the normalisation —
/// the actual HMAC construction (the inner double hash) is still entirely
/// the `hmac` crate's, via the exact-size [`Mac::new`], which takes no
/// `Result` at all because a matching key length cannot be wrong.
...
/// HMAC's inner double hash keys on a block-sized buffer — 64 bytes for
/// SHA-256 — per RFC 2104 §2: a key longer than the block is hashed down
...
/// source no longer than the 64-byte destination by construction.
fn block_sized_key(key: &[u8]) -> [u8; 64] {
