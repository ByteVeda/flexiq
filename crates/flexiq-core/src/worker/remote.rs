    ...
    fn spawn_drain(
        ...
        thread::Builder::new()
            ...
            .spawn(move || {
                ...
                if dropped > 0 {
                    log::warn!(
                        "[flexiq] dropped {dropped} executor task log line(s) that arrived \
...
impl Shared {
    ...
    fn attach(self: &Arc<Self>, transport: Box<dyn Transport>) -> Result<String, AttachError> {
        ...
        let ExecutorMessage::Hello {
            ...
            log::warn!("[flexiq] attach from {peer} sent a frame before hello; dropping");
        ...
        if let Some(expected) = &self.config.auth_token {
            if !token.is_some_and(|presented| expected.matches(&presented)) {
                ...
                log::warn!(
                    "[flexiq] rejecting executor {executor_id} ({sdk} {version}, {peer}): \
        ...
        log::info!("[flexiq] executor {executor_id} attached from {peer} with {slots} slot(s)");
        Ok(executor_id)
    }
    ...
    fn dispatch_to(
        ...
        if let Err(e) = write {
            ...
            executor.free.fetch_add(1, Ordering::Relaxed);
            log::error!(
                "[flexiq] failed to send job {} to executor {}: {e}; will be reaped",
                job.id,
                executor.id
            );
    ...
    fn fail_unplaceable(&self, job: &Job, reason: &str) {
        ...
        log::warn!("[flexiq] {error} (job {})", job.id);
        ...
        match sender {
            Some(tx) => {
                if tx.send(result).is_err() {
                    log::debug!("[flexiq] result channel closed; dropping executor result");
                }
            }
            None => log::warn!("[flexiq] result arrived before the dispatcher started; dropping"),
        }
    }
    ...
    fn spawn_reader(
        ...
        thread::Builder::new()
            .name(format!("flexiq-executor-{}", executor.id))
            .spawn(move || {
                    ...
                    match reader.read_or_skip::<ExecutorMessage>() {
                        ...
                        Ok(Incoming::Unknown { frame_type }) => {
                            ...
                            if reported_unknown.insert(frame_type.clone()) {
                                log::warn!(
                                    "[flexiq] executor {} sent a '{frame_type}' frame this \
                        ...
                        Err(ProtocolError::Eof) => {
                            log::info!("[flexiq] executor {} disconnected", executor.id);
                            break;
                        }
                        Err(e) => {
                            log::warn!("[flexiq] executor {} read error: {e}", executor.id);
                            break;
                        }
                    }
                }
    ...
    fn handle_frame(&self, executor: &Arc<Executor>, message: ExecutorMessage, payload: Vec<u8>) {
        ...
        let Some(result) = message.into_job_result(payload) else {
            log::warn!(
                "[flexiq] executor {} sent a handshake frame mid-stream",
                executor.id
            );
            return;
        };
        ...
        if !known {
            log::warn!(
                "[flexiq] executor {} returned a result for unknown job {}",
                executor.id,
                result.job_id()
            );
            return;
    ...
    fn apply_task_log(
        ...
        let extra = extra.and_then(|blob| match String::from_utf8(blob) {
            ...
            Err(_) => {
                log::warn!(
                    "[flexiq] executor {} sent a task log for job {job_id} whose extra blob is \
    ...
    fn side_channel_for(
        ...
        let Some(pump) = self.side_channel.as_ref() else {
            // Never advertised, so a correct executor never sent this.
            log::debug!(
                "[flexiq] executor {} sent a {what} frame but this scheduler advertised no \
        ...
        let Some(dispatched) = executor.running(job_id) else {
            log::warn!(
                "[flexiq] executor {} sent a {what} for job {job_id}, which it is not running; \
                 dropping",
                executor.id
            );
            return None;
    ...
    fn spawn_cancel_router(self: Arc<Self>, cancel_rx: Receiver<String>) -> JoinHandle<()> {
        thread::Builder::new()
            .name("flexiq-executor-cancel-router".into())
            .spawn(move || {
                for job_id in cancel_rx.iter() {
                    ...
                    if let Err(e) = sent {
                        log::warn!(
                            "[flexiq] failed to forward cancel for {job_id} to executor {}: {e}",
                            executor.id
                        );
                    }
                }
    ...
    async fn drain_and_close(&self) {
        ...
        for executor in &executors {
            ...
            if still_running > 0 {
                log::warn!(
                    "[flexiq] executor {} did not drain {still_running} job(s) within the \
