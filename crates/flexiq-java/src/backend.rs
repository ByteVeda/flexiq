...
use flexiq_core::{NewJob, SqliteStorage, StorageBackend};
...
#[cfg(feature = "postgres")]
const DEFAULT_POSTGRES_SCHEMA: &str = "flexiq";
...
pub struct QueueHandle {
    ...
    #[cfg(feature = "workflows")]
    pub workflow_storage: std::sync::OnceLock<flexiq_workflows::WorkflowStorageBackend>,
...
impl QueueHandle {
    ...
    pub fn workflow_store(&self) -> Result<flexiq_workflows::WorkflowStorageBackend, BindingError> {
...
pub(crate) fn build_workflow_storage(
    ...
    auto_migrate: bool,
) -> Result<flexiq_workflows::WorkflowStorageBackend, BindingError> {
    ...
    let wf = match storage {
        ...
        StorageBackend::Postgres(s) => WorkflowStorageBackend::Postgres(if auto_migrate {
            flexiq_workflows::WorkflowPostgresStorage::new(s.clone(), namespace)?
        } else {
            flexiq_workflows::WorkflowPostgresStorage::unmigrated(s.clone(), namespace)?
        }),
        #[cfg(feature = "redis")]
        StorageBackend::Redis(s) => WorkflowStorageBackend::Redis(
            flexiq_workflows::WorkflowRedisStorage::new(s.clone(), namespace)?,
        ),
    };
    Ok(wf)
}
...
pub fn enqueue_batch_dedup(
    ...
    let created = flexiq_core::storage::enqueue_batch_dedup(storage, jobs)?;
...
pub fn reap_ephemeral_subscriptions(storage: &StorageBackend) -> Result<u64, BindingError> {
    Ok(flexiq_core::storage::sweep_ephemeral_subscriptions(
        storage, None,
    )?)
}
    ...
    let storage = match options.backend.as_deref().unwrap_or("sqlite") {
        ...
        "postgres" => {
            ...
            StorageBackend::Postgres(if auto_migrate {
                flexiq_core::PostgresStorage::with_schema_and_pool_size(&options.dsn, schema, pool)?
            } else {
                flexiq_core::PostgresStorage::unmigrated(&options.dsn, schema, pool)?
            })
        }
        #[cfg(feature = "redis")]
        "redis" => {
            let storage = match options.prefix.as_deref() {
                Some(prefix) => flexiq_core::RedisStorage::with_prefix(&options.dsn, prefix),
                None => flexiq_core::RedisStorage::new(&options.dsn),
            }?;
