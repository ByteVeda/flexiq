...
use crate::executor::AttachedHandle;
use crate::ffi::{guard, read_string};
use crate::handle::{self, into_handle};
use crate::steps::{step_error, StepSessionHandle};
...
///
/// The payload is *not* kept: it is moved out before a job lands here, so an
/// entry costs a handful of strings rather than a copy of however many megabytes
...
impl RunningJobs {
    ...
    fn locked(&self) -> MutexGuard<'_, HashMap<String, Job>> {
...
/// `boolean supportsSteps(long handle)` — whether durable steps work across
/// this attach.
///
/// False against a scheduler whose storage has no step store, or one built
/// before steps existed. Not a gate — `openStepSession` refuses on its own — but
/// it lets the shell say so once at attach rather than at the first `step.run`.
#[no_mangle]
pub extern "system" fn Java_org_byteveda_flexiq_internal_NativeExecutor_supportsSteps<'local>(
    mut env: JNIEnv<'local>,
    _class: JClass<'local>,
    handle: jlong,
) -> jboolean {
    guard(&mut env, 0, |_env| {
        let executor = unsafe { handle::borrow::<AttachedHandle>(handle) };
        Ok(jboolean::from(executor.steps.is_supported()))
    })
}

/// `long openStepSession(long executorHandle, String jobId, int attempt)`.
///
/// On the **executor**, not a queue: an executor's app module builds a queue
/// with no storage behind it, and the job belongs to the scheduler either way.
///
/// Both arguments are checked against the dispatch rather than trusted, for the
/// same reason the worker twin re-reads the job row: a session opened for a job
/// this executor is not running, or for an attempt it has already reported,
/// would commit into a sequence that is not its own.
///
/// A scheduler that never advertised the step capability is refused by the
/// core's own `open_session`, retryably — a fleet mid-rollout may place the next
/// attempt somewhere that can commit. Restating that rule here is how a shell's
/// copy of it drifts.
#[no_mangle]
pub extern "system" fn Java_org_byteveda_flexiq_internal_NativeExecutor_openStepSession<'local>(
    mut env: JNIEnv<'local>,
    _class: JClass<'local>,
    handle: jlong,
    ...
    guard(&mut env, 0, |env| {
        ...
        Ok(into_handle(open_step_session(executor, &job_id, attempt)?))
    })
}

/// Open one attempt's session over the scheduler connection.
fn open_step_session(
    executor: &AttachedHandle,
    job_id: &str,
    attempt: jint,
) -> Result<StepSessionHandle, BindingError> {
    let job = executor.running.get(job_id).ok_or_else(|| {
        // Reported as a lost claim, which is what it is: this executor is not
        // running that attempt, so anything it wrote would be refused by the
        // scheduler's own fence.
    ...
    // The defaults, as the worker twin uses: §4.2's answer to a result that will
    // not fit is to store it elsewhere and memoize the handle, so there is
    // nothing for a shell knob to do. The caps that *hold* are the scheduler's,
