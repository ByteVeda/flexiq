...
/// alone".
pub const INJECT: &str = "taskito.dev/inject";
/// Scheduler address the executor dials.
pub const ATTACH: &str = "taskito.dev/attach";
/// Argv that runs an executor inside the app image.
pub const COMMAND: &str = "taskito.dev/command";
/// Jobs the executor runs at once.
pub const SLOTS: &str = "taskito.dev/slots";
/// Container whose image and environment the sidecar copies.
pub const CONTAINER: &str = "taskito.dev/container";
/// Secret holding the attach token.
pub const TOKEN_SECRET: &str = "taskito.dev/token-secret";
/// Key within that secret.
pub const TOKEN_KEY: &str = "taskito.dev/token-key";
/// Volume to mount for a `unix:` attach address.
pub const SOCKET_VOLUME: &str = "taskito.dev/socket-volume";
/// Whether to copy the source container's `env` and `envFrom`.
pub const INHERIT_ENV: &str = "taskito.dev/inherit-env";
...
mod tests {
    ...
    fn a_pod_without_the_switch_is_left_alone() {
        let spec = parse(&annotations(&[(ATTACH, "taskito-scheduler:7777")])).expect("valid");
        assert!(spec.is_none());
    }
    ...
    fn the_minimal_opt_in_parses_with_defaults() {
        let spec = parse(&annotations(&minimal()))
            .expect("valid")
            .expect("opted in");
        assert_eq!(spec.attach, "taskito-scheduler:7777");
        assert_eq!(
            spec.command,
            vec!["taskito", "executor", "--app", "myapp:queue"]
        );
    ...
    fn a_unix_address_without_a_volume_is_rejected() {
        let mut pairs = minimal();
        pairs[1] = (ATTACH, "unix:/run/taskito/attach.sock");
        let error = parse(&annotations(&pairs)).expect_err("must reject");
    ...
    fn a_token_key_defaults_when_only_the_secret_is_named() {
        let mut pairs = minimal();
        pairs.push((TOKEN_SECRET, "taskito"));
        let spec = parse(&annotations(&pairs))
            .expect("valid")
            .expect("opted in");
        assert_eq!(
