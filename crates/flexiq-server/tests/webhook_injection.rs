...
use flexiq_server::config::webhook::WebhookConfig;
use flexiq_server::webhook::{router, MUTATE_PATH};
...
async fn an_annotated_pod_is_admitted_with_a_sidecar_patch() {
    ...
    assert_eq!(container["value"]["name"], "flexiq-executor");
...
async fn a_pod_missing_a_required_annotation_is_denied_with_the_reason() {
    ...
    let message = body["response"]["status"]["message"]
        ...
        .expect("a message");
    assert!(
        message.contains("flexiq.dev/command"),
...
async fn re_admitting_an_injected_pod_adds_nothing() {
    let already = json!({
        ...
        "spec": { "containers": [
            ...
            { "name": "flexiq-executor", "image": "myapp:1.4.2" },
        ]},
    });
