...
mod tests {
    ...
    fn csrf_requires_cookie_header_and_session_to_agree() {
        let request = headers(&[("cookie", "flexiq_csrf=abc"), ("x-csrf-token", "abc")]);
        let context = RequestContext::new(&request, Some(session("abc")));
        assert!(context.csrf_valid());

        // Header missing.
        let context = RequestContext::new(
            &headers(&[("cookie", "flexiq_csrf=abc")]),
            Some(session("abc")),
        );
        assert!(!context.csrf_valid());
    ...
    fn the_session_token_comes_from_its_own_cookie() {
        let request = headers(&[("cookie", "other=1; flexiq_session=tok; flexiq_csrf=abc")]);
