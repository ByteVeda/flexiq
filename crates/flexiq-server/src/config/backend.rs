...
//!
//! Same dispatch as `crates/flexiq-tui/src/backend.rs`, plus the explicit
    ...
    if !auto_migrate && !backend.storage.is_migrated()? {
        bail!(
            "FLEXIQ_AUTO_MIGRATE is off, so this server applies no schema, and this \
             database has none — run an SDK's `flexiq migrate` against it first"
        );
    }
    flexiq_core::ensure_contract_supported(&backend.storage)?;
    Ok(backend)
}
...
mod tests {
    ...
    fn a_gated_open_applies_no_ddl_and_says_so_when_the_schema_is_absent() {
        ...
        let dir =
            std::env::temp_dir().join(format!("flexiq-server-gated-{}", flexiq_core::now_millis()));
