...
impl JsQueue {
    ...
    pub async fn dead_letters_after(
        ...
        spawn_blocking(move || {
            ...
            let dead = storage
                ...
                .map_err(to_napi_err)?;
            let next_cursor =
                flexiq_core::storage::cursor::next_cursor(&dead, limit, |d| (d.failed_at, &d.id));
            Ok(JsDeadJobPage {
    ...
    pub fn min_contract(&self) -> Result<u32> {
        flexiq_core::min_contract(&self.storage).map_err(to_napi_err)
    }
    ...
    pub fn set_min_contract(&self, level: u32) -> Result<()> {
        flexiq_core::set_min_contract(&self.storage, level).map_err(to_napi_err)
    }
    ...
    pub async fn dry_run_retention(&self, retention: Option<RetentionInput>) -> Result<String> {
        ...
        spawn_blocking(move || {
            match config {
                Some(config) => flexiq_core::scheduler::retention::dry_run_json(
                    &storage,
                    Some(&config),
                    None,
                    namespace.as_deref(),
...
fn migrate_workflow_storage(
    storage: &flexiq_core::StorageBackend,
...
fn migrate_workflow_storage(
    _storage: &flexiq_core::StorageBackend,
