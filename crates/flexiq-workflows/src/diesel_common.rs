...
macro_rules! impl_workflow_diesel_ops {
    ($storage_type:ty, $conn_type:ty, $prep_sql:path) => {
        impl $storage_type {
            ...
            fn run_out_of_scope(&self, run_id: &str) -> ::flexiq_core::error::Result<bool> {
        ...
        impl $crate::storage::WorkflowStorage for $storage_type {
            fn create_workflow_definition(
                ...
                let meta_json = ::serde_json::to_string(&def.step_metadata).map_err(|e| {
                    ::flexiq_core::error::QueueError::Serialization(format!(
            ...
            fn get_workflow_definition(
                ...
                name: &str,
                version: Option<i32>,
            ) -> ::flexiq_core::error::Result<Option<$crate::WorkflowDefinition>> {
                let mut conn = self.inner.conn()?;
            ...
            fn get_workflow_run(
                &self,
                run_id: &str,
            ) -> ::flexiq_core::error::Result<Option<$crate::WorkflowRun>> {
                let mut conn = self.inner.conn()?;
            ...
            fn list_workflow_runs(
                ...
                limit: i64,
                offset: i64,
            ) -> ::flexiq_core::error::Result<Vec<$crate::WorkflowRun>> {
                let mut conn = self.inner.conn()?;
            ...
            fn create_workflow_nodes_batch(
                ...
                let mut conn = self.inner.conn()?;
                conn.transaction::<_, ::flexiq_core::error::QueueError, _>(|conn| {
                    for node in nodes {
                        ::diesel::sql_query(
                            &$prep_sql("INSERT INTO workflow_nodes
                                (id, run_id, node_name, job_id, status, result_hash,
            ...
            fn get_workflow_node(
                ...
                run_id: &str,
                node_name: &str,
            ) -> ::flexiq_core::error::Result<Option<$crate::WorkflowNode>> {
            ...
            fn get_workflow_nodes(
                &self,
                run_id: &str,
            ) -> ::flexiq_core::error::Result<Vec<$crate::WorkflowNode>> {
                if self.run_out_of_scope(run_id)? {
                    return Ok(Vec::new());
                }
                let mut conn = self.inner.conn()?;
