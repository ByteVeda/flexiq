...
# The OpenAPI document is generated from the descriptor by a workspace member,
# so it is gated here rather than in a Rust job: both artifacts go stale for the
# same reason — someone edited a .proto — and one gate should say so once.
# `--manifest-path` because this script runs from inside contracts/proto.
generate_openapi() {
  cargo run --quiet --locked --manifest-path "$repo_root/Cargo.toml" \
    -p flexiq-openapi -- --out "$1"
}
...
if ! command -v cargo >/dev/null 2>&1; then
  echo "error: cargo is not on PATH, and contracts/openapi.json is generated by" >&2
  echo "error: a workspace member. Install the toolchain: https://rustup.rs" >&2
  exit 1
fi
...
if [ "$fix" = true ]; then
  ...
  # After the descriptor, never beside it: the document is generated from the
  # bytes buf just wrote.
  generate_openapi "$openapi"
  echo "proto-check: formatted, linted, descriptor and OpenAPI document regenerated."
  exit 0
fi
...
# The document is generated from the committed descriptor, which the comparison
# above has just shown to be the protos'. So a mismatch here is the document's
# staleness alone, never the descriptor's.
regenerated_openapi="$(mktemp)"
trap 'rm -f "$regenerated" "$regenerated_openapi"' EXIT
generate_openapi "$regenerated_openapi"

if [ ! -f "$openapi" ]; then
  echo "error: contracts/openapi.json is missing. Run: $0 --fix" >&2
  exit 1
fi

if ! cmp -s "$regenerated_openapi" "$openapi"; then
  echo "error: contracts/openapi.json is stale — it does not match the contract." >&2
  echo "error: regenerate it with: $0 --fix" >&2
  exit 1
fi

echo "proto-check: format, lint, descriptor and OpenAPI document all clean."
