...
**TL;DR**: FlexiQ is <SdkSwap python="Celery" node="BullMQ" java="a task queue" /> without the
...
{/* Outside the three blocks above on purpose. Every SDK has an
    `operate/migration` page now, so one <SdkLink> resolves per SDK — and this
    is the only spot on the page all three readers reach. */}

Looking to switch? See the{" "}
<SdkLink to="operate/migration">
  Migrating from <SdkSwap python="Celery" node="BullMQ" java="JVM job scheduling" />
</SdkLink>{" "}
