---
title: Comparison
description: "FlexiQ vs other task queues — feature matrix and decision guide."
---

**TL;DR**: FlexiQ is <SdkSwap python="Celery" node="BullMQ" java="a task queue" rust="a task queue" /> without the
broker (a separate server such as Redis or RabbitMQ that holds pending jobs). Rust scheduler, no broker required, and no separate service to operate.
Start with SQLite; add Postgres or Redis as a backend when you scale out.

The page below compares features. For speed, see [Benchmarks](/about/benchmarks) —
measured, reproducible, and not flattering on every axis.
...
## When NOT to use flexiq

Consider alternatives when:

- **Multi-server workers** — you need workers on separate machines (flexiq supports this with Postgres/Redis backends, but a broker-based queue has more mature distributed tooling)
...
- **Complex routing** — you need topic exchanges, message filtering, or fan-out patterns beyond flexiq's pub/sub
...
**Choose flexiq** if you want zero-infrastructure simplicity on a single machine.
...
**Choose flexiq** if you want similar simplicity without requiring Redis.
...
**Choose flexiq** if you want built-in DLQ and priority without a broker.
...
**Choose flexiq** if you want higher performance and more features with SQLite.
...
Choose flexiq if you want zero infrastructure. Choose TaskIQ if you're fully
...
Most Node task queues need a server beside your app: **Bee-Queue** and **Bull**
require Redis, **BullMQ** requires Redis (or, since version 6, PostgreSQL), and
**Agenda** requires MongoDB. flexiq embeds the queue in a Rust core with a
pluggable backend (SQLite, Postgres, or Redis), so you can start with **zero
infrastructure** and add a server only when you scale out.

|  | flexiq | BullMQ / Bee-Queue | Agenda |
|---|---|---|---|
| **Server** | None (SQLite) — optional Postgres/Redis | Redis; BullMQ 6 also PostgreSQL | MongoDB required |
...
**Choose flexiq** for a broker-optional queue with a workflow engine built in.
...
batch/ETL pipelines. flexiq is a general-purpose task queue with a Rust core:
...
**Choose flexiq** for a broker-optional queue with priorities, DLQ, and a
...
{/* Outside the four blocks above on purpose. Every SDK has an
    `operate/migration` page now, so one <SdkLink> resolves per SDK — and this
    is the only spot on the page all four readers reach. */}

Looking to switch? See the{" "}
<SdkLink to="operate/migration">
  Migrating from <SdkSwap python="Celery" node="BullMQ" java="JVM job scheduling" rust={<code>flexiq-core</code>} />
</SdkLink>{" "}
