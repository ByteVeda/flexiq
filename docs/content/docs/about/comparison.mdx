...
{/* Split per SDK rather than one <SdkLink>: the migration guide is written per
    source queue, and not every SDK has one to point at. */}

<SdkOnly sdk="python">
Looking to switch? See the [Migrating from Celery](/python/operate/migration)
guide for a step-by-step walkthrough with side-by-side code examples.
</SdkOnly>

<SdkOnly sdk="node">
Looking to switch? See the [Migrating from BullMQ](/node/operate/migration)
guide for a step-by-step walkthrough with side-by-side code examples.
</SdkOnly>
