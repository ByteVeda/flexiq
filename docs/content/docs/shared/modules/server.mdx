...
that enqueues over the gRPC door with `grpcurl`, no SDK installed at all. The
workers still open the database file directly in that variant — only the
producer moved to the network door.
