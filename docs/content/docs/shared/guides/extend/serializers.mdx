...
travels in CBOR's shortest form, while an `f32` is widened to a 64-bit float
because the wire contract pins that width for every float, so the narrower type
buys nothing on the wire.
