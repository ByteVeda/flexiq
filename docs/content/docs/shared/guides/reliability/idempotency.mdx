...
Coalescing is one of three collapse rules; <SdkLink to="guides/reliability/flow-control">Flow control</SdkLink>
covers how it differs from a debounce window and from a rate limit.

<Callout type="info" title="Deduping the enqueue, not the effect">
  This key collapses duplicate *enqueues*. It does nothing about a task body
  that already charged a card and then crashed before committing — for that you
  need a key the downstream service dedupes on, minted per checkpoint. See
...
```rust
let first = queue.enqueue(charge_customer::call(42, 1_000).unique_key("charge:42:1000"))?;
let second = queue.enqueue(charge_customer::call(42, 1_000).unique_key("charge:42:1000"))?;

assert_eq!(first.id, second.id); // coalesced onto the first — no duplicate created
```
...
Where a binding has an idempotent flag, it derives this key for you instead of
you building the string by hand: mark a task idempotent and FlexiQ hashes the
task name and arguments into a dedup key on every enqueue.
...
`.idempotent()` on the call derives the key, and `#[flexiq::task(idempotent)]`
makes it the default for every call of that task:

```rust
#[flexiq::task(idempotent)]
fn charge_customer(customer_id: i64, amount_cents: i64) -> flexiq::Outcome<()> {
    Ok(())
}

let first = queue.enqueue(charge_customer::call(42, 1_000))?;
let second = queue.enqueue(charge_customer::call(42, 1_000))?;

assert_eq!(first.id, second.id); // second call coalesced into the first
```

An explicit `.unique_key(..)` on the same call beats `.idempotent()`.
...
<SdkLink to="guides/extend/serializers">serializer</SdkLink> would
...
<Callout type="info" title="Identical recipe across bindings">
  Every binding that derives a key computes this exact same one for the same
  `(task_name, payload)` pair, so idempotent enqueues from any of them dedupe
  against each other when they share one queue.
</Callout>
...
The crate pins the digest rather than merely asserting it is stable against
itself: a test asserts that one known call derives
`auto:c3abb8ce58c62cb737c17b92beb816be`, the exact string a producer in another
language computes for the same `(task_name, payload)` pair. Getting the NUL
separator or the 32-character truncation wrong still enqueues — it only stops
deduping across the boundary — so the expected value is written out where a
change to it fails the build.

</SdkOnly>

## A key is scoped to one namespace

Two jobs collide on a key only when they share a namespace. An unnamespaced job
is its own group, distinct from a job in a namespace called `""`. This makes a
key a per-tenant fact rather than a global one, so two tenants using the same
natural key — an order id, a customer id — do not suppress each other's work.

<Callout type="warn" title="This changed in 2.0.0">
  Before 2.0.0 the index was on the key alone, so a key reached across every
  namespace: the second tenant to use it got the first tenant's job back and
  never created its own. If you relied on that reach, key on something that is
  namespace-independent instead. A deployment with a single namespace, or none,
  is unaffected.
</Callout>
...
Two inputs, on `EnqueueOptions` and on `TaskCall` alike:

| Setter | Effect |
|---|---|
| `.unique_key("…")` | The literal storage key. Highest precedence — wins over the line below. |
| `.idempotent()` | Derive the key from the task name and payload, when no explicit key was set. |

There is no `idempotency_key` alias, and no way to turn derivation *off* for one
call — `.idempotent()` takes no argument and only ever sets the flag. A task
declared `#[flexiq::task(idempotent)]` therefore dedupes on every call unless
that call names its own `.unique_key(..)`.

</SdkOnly>

## Bulk enqueues

Batch enqueue trades the per-row dedup check for throughput, and the bindings
don't all behave the same way when a batch contains a colliding key.
...
// ids[0] === ids[1] — one job, the duplicate maps to it, in input order
```

A batch carrying no `uniqueKey` at all keeps the plain bulk insert, so it costs
...
`enqueue_batch` switches the whole batch onto the unique check-then-insert path
as soon as **one** call in it carries a key, explicit or derived. A duplicate
inside the batch, or one colliding with an already-active job, resolves to the
existing job instead of failing the call:

```rust
let jobs = queue.enqueue_batch(vec![
    charge_customer::call(42, 1_000).idempotent(),
    charge_customer::call(42, 1_000).idempotent(),
])?;
// the duplicate resolves onto the already-active job rather than failing
```

A batch where no call carries a key keeps the plain bulk insert — one
transaction either way. A call carrying a *debounce window* is the one thing a
batch refuses outright, because storage has no batched debounce to hand it to;
enqueue that call on its own.
...
- **Don't derive a key from a moving value.** An argument holding a timestamp or
  a freshly generated id hashes differently on every call, so `.idempotent()`
  never dedupes. Generate it inside the body, or pass a stable identifier.
- **Argument order is part of the hash, and so is struct field order.** The
  digest runs over the encoded payload, and the encoder keeps map keys in
  insertion order on purpose — sorting them behind the caller's back would
  change an identity. Reordering a task's parameters, or the fields of a type it
  takes, changes what dedupes against what.
- **There is no serializer to swap.** The payload codec is the wire CBOR and it
  is fixed, so the hash cannot drift from a codec change between releases. Use
  an explicit `.unique_key(..)` for a key that has to survive a change to the
  *arguments* themselves.
- **A debounce window silently beats a key.** `enqueue` matches on the window
  before it looks at the dedup key, so a call carrying both never derives the
  `auto:` key and never runs a dedup lookup at all.
