...
<Callout type="warn" title="A shared secret is not a finished credential">
`FLEXIQ_GRPC_TOKEN` is one secret every client presents. It cannot be revoked
for one client, it carries no scope, and it leaves no audit trail. It is enough
to keep an anonymous caller off an `Enqueue` port, and it is not enough to put
that port on an untrusted network: terminate TLS in front of the listener and
keep it reachable only from networks you already trust, until scoped tokens
land.
</Callout>
...
listener uses, so the shared **group** is again the access control — which is
why a Unix socket needs no token even though it is not a loopback address.
...
```bash
# $FLEXIQ_GRPC_TOKEN is the value the listener was started with — see
# "The credential" below for where it comes from.
...
`FLEXIQ_GRPC_TOKEN` is the secret a caller presents, as an `authorization`
metadata entry:

```bash
# Exported rather than prefixed onto the command: a `VAR=x cmd` assignment
# reaches that one process, and a client on the same shell needs the value too.
...
It must be at least 16 characters — the same floor `FLEXIQ_ATTACH_TOKEN` has,
because a guessable secret is not a control — and the server removes it from its
own environment once parsed, so a task body or a crash dump cannot read it back.

**A non-loopback bind without it is refused at boot.** Loopback and Unix sockets
still need no token: there the network stack and the filesystem mode are the
boundary. Setting one on a loopback bind is honoured rather than optimised away.

**One check covers the whole listener**, not one per RPC. With a token
configured, a wrong one, a missing one, one under the wrong scheme and a call to
an RPC that does not exist all answer `UNAUTHENTICATED` with the reason
`UNAUTHENTICATED` and the same message — nothing distinguishes them, because a
...
carries. On a tokenless loopback or Unix-socket listener there is nothing to
guess: every caller is accepted, so an unknown RPC reaches the router and comes
back `UNIMPLEMENTED` as it would on any gRPC server.
...
`flexiq.v1` has a namespace field, in any RPC. Under a shared secret that
namespace is the server's own `FLEXIQ_NAMESPACE`, so a token is implicitly
scoped to the one namespace this process schedules.
...
- **The credential is a shared secret, and it is one factor.** It proves the
  peer knows a string; it does not encrypt the connection, it cannot be revoked
  for one client, and it grants everything. A non-loopback bind without it is
  refused at boot rather than allowed with a warning, because an unauthenticated
  port that accepts `Enqueue` is not a thing to leave running behind a log line.
...
`grpc.enabled` needs `grpc.token` or `grpc.existingSecret`, refused at `helm
template` time rather than rendering a Deployment that CrashLoopBackOffs: the
chart binds `0.0.0.0` because kubelet dials the pod IP for every probe type, and
the server refuses that bind without a credential.

```yaml
namespace: prod
grpc:
  enabled: true
  token: "" # or existingSecret, holding key `grpc-token`
```
