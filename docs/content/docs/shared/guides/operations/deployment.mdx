...
spec:
  template:
    spec:
      # The socket is created with the binding process's umask (mode 0755), and
      # connect(2) needs write permission on it — so both containers run as the
      # same uid. This is the whole access boundary for a Unix-socket attach.
