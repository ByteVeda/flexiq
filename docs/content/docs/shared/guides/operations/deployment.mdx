...
<Callout type="warn" title="Migrate before you serve">
  ...
  then. `taskito-server` honors the same switch through `TASKITO_AUTO_MIGRATE`,
  and refuses to start against a database whose schema is missing rather than
  leaking a bare "no such table".
</Callout>
