...
```java
try (Taskito taskito = Taskito.builder().sqlite("/var/lib/myapp/flexiq.db").open();
...
```java
Taskito taskito = Taskito.builder().sqlite("/var/lib/myapp/flexiq.db").open();
```
...
<Callout type="warn" title="FLEXIQ_DB_PATH is not read automatically">
  The `taskito` CLI and the `Queue` constructor do not read a `FLEXIQ_DB_PATH`
...
services:
  ...
  dashboard:
    build: .
    command: node_modules/.bin/taskito --db /data/flexiq.db dashboard --host 0.0.0.0
    volumes:
      - taskito-data:/data
    ports:
      - "8787:8787"
...
services:
  ...
  dashboard:
    build: .
    command: java -cp app.jar org.byteveda.taskito.cli.Cli --url /data/flexiq.db dashboard --port 8080
    volumes:
      - taskito-data:/data
    ports:
      - "8080:8080"
...
services:
  scheduler:
    ...
    environment:
      FLEXIQ_DSN: postgresql://taskito:secret@postgres:5432/myapp
...
Executors then set `FLEXIQ_ATTACH: taskito-scheduler:7777` and read
...
sqlite3 /var/lib/myapp/flexiq.db ".backup /backups/taskito-$(date +%Y%m%d).db"

# Option 2: SQLite VACUUM INTO command
sqlite3 /var/lib/myapp/flexiq.db "VACUUM INTO '/backups/taskito-$(date +%Y%m%d).db';"
```
...
services:
  ...
  worker:
    ...
    environment:
      ...
      FLEXIQ_DB_URL: postgresql://taskito:secret@postgres:5432/myapp
    depends_on:
      - postgres
    stop_signal: SIGINT
    stop_grace_period: 35s
...
services:
  ...
  worker:
    build: .
    environment:
      FLEXIQ_PG_URL: postgresql://taskito:secret@postgres:5432/myapp
    depends_on:
      - postgres
    stop_signal: SIGTERM
    stop_grace_period: 10s
...
<Callout type="info">
  ...
  `Taskito.builder().postgres(System.getenv("FLEXIQ_PG_URL")).open()`); the
...
```bash
taskito --db flexiq.db dashboard --host 0.0.0.0 --port 8787
```
...
```bash
taskito --url flexiq.db dashboard --port 8080
```
...
<SdkSwap python={<code>Queue(auto_migrate=False)</code>} node={<code>new Queue(&#123; autoMigrate: false &#125;)</code>} java={<code>Taskito.builder().autoMigrate(false)</code>} />
...
<Callout type="warn" title="Migrate before you serve">
  ...
  then. `taskito-server` honors the same switch through `FLEXIQ_AUTO_MIGRATE`
...
<SdkSwap python={<code>queue.min_contract()</code>} node={<code>queue.minContract()</code>} java={<code>taskito.minContract()</code>} />
...
<SdkSwap python={<code>queue.set_min_contract(2)</code>} node={<code>queue.setMinContract(2)</code>} java={<code>taskito.setMinContract(2)</code>} />
