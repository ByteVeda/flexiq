...
**One check covers the whole listener**, not one per RPC. A wrong token, a
missing one, a token under the wrong scheme and a call to an RPC that does not
exist all answer `UNAUTHENTICATED` with the reason `UNAUTHENTICATED` and the
same message — nothing distinguishes them, because a distinguishable answer
tells a client guessing tokens which guess was closer, and a path answered
before authentication tells it which services the build carries.

**`grpc.health.v1` is the one exception** and is answered without a credential.
