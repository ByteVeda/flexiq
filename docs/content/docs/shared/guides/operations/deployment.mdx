...
Three things the surface deliberately does not offer. There is **no admin
...
operator surface and a producer surface should not share a credential. There is
**no way to name a namespace**; see below. And `raw` is the tagged envelope from
the binding contract, carried opaquely — a client that cannot build one needs
the structured-argument door, which is not here yet.
