...
fourth beside attach, the dashboard and the admission webhook. It is where the
network-side producer door will live — enqueue over gRPC rather than over a
database connection, holding a token rather than a DSN — and **today it serves
`grpc.health.v1` and server reflection only**. The FlexiQ services land on this
listener as they arrive; no caller is authenticated yet.
...
listener uses, so the shared **group** is again the access control.
...
Two services answer today; the FlexiQ services arrive on the same listener as
they land.
...
- **No caller is authenticated yet.** Until token authentication lands, treat a
  non-loopback bind as reachable by anything on that network; the server logs a
  warning at boot saying so. Keep the port inside the cluster, the way the attach
  port already is.

### With Helm

```bash
helm upgrade --install flexiq ./deploy/helm/flexiq-server \
  --set storage.dsn='postgres://flexiq:secret@postgres:5432/myapp' \
  --set attach.token="$ATTACH_TOKEN" \
  --set namespace=prod \
  --set grpc.enabled=true
```

The door gets its own `ClusterIP` Service — never merged with the dashboard's,
for the same reason attach is separate — with `appProtocol: grpc` so a mesh or
an Ingress keyed off it speaks HTTP/2 rather than downgrading and breaking every
stream. `grpc.enabled` without `namespace` fails at `helm template` time, with
the reason, rather than at the pod's first restart.
