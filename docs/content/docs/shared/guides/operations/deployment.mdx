...
<Callout type="info" title="The probes live on the dashboard port">
  `/health` and `/readiness` are dashboard routes, so probing needs
  `TASKITO_DASHBOARD` set — a listener-only server has no HTTP port to probe,
  and a `tcpSocket` probe on the attach port is the fallback. If you set
  `TASKITO_DASHBOARD_METRICS_TOKEN`, `/readiness` starts requiring that bearer
  token; probe `/health` only, or pass the header. `/readiness` reports
  `workers: none` on an attach deployment — executors are not workers — and
  still answers 200.
</Callout>
