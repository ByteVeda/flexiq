...
network-side producer door: a client enqueues over gRPC instead of over a
database connection, so the credential it holds is a token rather than a DSN.
...
how the Helm chart wires a gRPC-only release.
