...
build the identical envelope, so a shell script or a `curl` one-liner can
enqueue work with no codec at all. Nothing downstream can tell the two apart:
the row is the same, and so are its bytes.
