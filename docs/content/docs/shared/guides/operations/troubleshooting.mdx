...
names dead-letter depending on which peer they land on.
...
**Cause**: two <SdkLink to="guides/operations/executor">attached
executors</SdkLink> advertised different task lists. The scheduler dispatches a
task name only to peers that advertised it, but a name only one peer knows is a
fatal, non-retryable failure wherever it lands on the other. `(none)` on one
side means that registry is a strict subset of the other's — the shape a
half-finished discovery walk produces.
...
image does not. Check the dead-letter queue for those names and requeue once the
registries agree. The warning is a diagnostic, never a gate — a divergent
executor still attaches and still receives work.
