...
many run at once on this worker; unset, it is unbounded, and the worker claims
jobs faster than it runs them, stranding them `running` where other workers
cannot take them. `batchSize` controls how many jobs are claimed per poll, and
