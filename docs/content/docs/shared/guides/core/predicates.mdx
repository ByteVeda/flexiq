...
to try again later.
<SdkSwap python={<>Register one with the <code>predicate=</code> option on <code>@queue.task</code>.</>} node={<>Register one with <code>queue.gate()</code>, returning a boolean or a richer <code>Decision</code>.</>} java={<>Register one with <code>taskito.predicate()</code>, or the richer <code>taskito.gate()</code>.</>} />
...
```ts
import { allOf, anyOf, businessHours, isWeekend, not } from "@byteveda/taskito";
...
| `false` / `Decision.reject(reason)` | Block it: `enqueue` throws `PredicateRejectedError`. |
...
```ts
import { Decision } from "@byteveda/taskito";
...
running, use <SdkLink to="guides/extensibility/middleware">middleware</SdkLink>
or <SdkLink to="guides/workflows/conditions">conditions</SdkLink> inside a
workflow.
...
Import the recipes individually, or as the `Recipes` bundle. Time-based recipes
defer an out-of-window enqueue to the next open moment; filters skip silently.
...
```ts
import { Recipes } from "@byteveda/taskito";
...
```ts
import { featureFlag, type FeatureFlagProvider } from "@byteveda/taskito";
...
```ts
import { defaultRegistry, registerPredicate, Recipes } from "@byteveda/taskito";
...
They're also <SdkLink to="guides/observability/webhooks">webhook</SdkLink>-subscribable.
...
```ts
import { anyOf, businessHours } from "@byteveda/taskito";
