...
A periodic is refused on a namespaced worker, and every method below is refused
on a namespaced handle: the table is keyed by name alone in every backend, so a
namespaced call would reach some other namespace's rows.
...
backend can manage it — including a producer that never starts a worker. All four
are also **refused on a namespaced handle**, and return an error saying so: the
table is keyed by name alone in every backend, so a namespaced listing would
return other namespaces' schedules and a delete would reach a name it does not
own.
