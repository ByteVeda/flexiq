...
An unchanged declaration writes nothing on restart, so a worker coming back up
never resets a deadline the scheduler advanced or resumes a task an operator
paused.
...
Re-registering an **unchanged** declaration writes nothing at all. That is what
stops a restart from resetting a `next_run` another worker's scheduler had
advanced, or from resuming a task an operator paused.
