...
A restart writes only what the declaration itself carries, so a worker coming
back up never resumes a task an operator paused and never resets a deadline the
scheduler advanced. The one exception is a declaration whose cron expression or
timezone changed: that does replace the deadline, because the stored one was
...
in the worker. What the declaration carries is written; `enabled` is not part of
it and is never written, and `next_run` is written only when the cron expression
or the timezone changed. So a restart cannot resume a task an operator paused,
whatever the declaration says, and cannot reset a `next_run` another worker's
scheduler advanced unless the schedule it was computed from is gone.
