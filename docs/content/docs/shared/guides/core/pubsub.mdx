...
<SdkSwap python={<>Subscribe with the <code>@queue.subscriber()</code> decorator.</>} node={<>Subscribe with <code>queue.subscriber()</code>.</>} java={<>Subscribe with <code>flexiq.subscribe()</code>.</>} />
...
<Callout type="info" title="Cross-SDK payload contract">
  ...
  configure the <SdkLink to="guides/extend/serializers">`CborSerializer`</SdkLink>
