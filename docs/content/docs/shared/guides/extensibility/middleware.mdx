...
middleware that caused it and the chain carries on. Failing an attempt over its
instrumentation is the failure mode these hooks exist to avoid — and a hook that
throws *on its own* still behaves exactly as [Exception handling](#exception-handling)
describes.
