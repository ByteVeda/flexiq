...
That listener refuses every call until a token exists, loopback included. Mint
one first — <SdkLink to="operate/server/tokens">API tokens</SdkLink> is the whole
lifecycle, and the rest of this page assumes `$FLEXIQ_TOKEN` holds one.
