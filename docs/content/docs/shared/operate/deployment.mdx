...
`kubectl debug --target=<container>`, which runs a tooling image as a separate
ephemeral container joined to the server's process namespace and leaves the
distroless target unchanged, instead of expecting `docker exec ... sh` to work.
...
local chart consumption, role selection, maintenance ownership across replicas,
sidecar injection, and KEDA.
