...
The <SdkLink to="modules/server">server module reference</SdkLink> lists its
...
<Callout type="info" title="A token proves who is calling, not that the line is private">
Callers present a scoped API token, and the door refuses every call that does
not. What a bearer token does not do is encrypt the connection — the listener
terminates no TLS, the same posture the attach listener takes. Put TLS in a
sidecar proxy or a service mesh in front of it.
</Callout>
...
That listener refuses every call until a token exists, loopback included — see
...
Mint the first one wherever the server can reach its database — no listener has
to be running:
...
dump, or from anyone with admin access. If it is lost, revoke it and issue
another.

The token is the *only* thing on stdout — the confirmation goes to stderr — so a
command substitution captures the credential and nothing else:

```bash
export FLEXIQ_TOKEN=$(flexiq-server token create --name my-producer --scope produce)
```

Then run the door and present it:
...
for longer is refused rather than quietly clamped. Once a token is inside 30, 20
and 10 days of expiry, the server logs a warning the first time it is *used* at
each threshold, so the credentials that get a reminder are the ones actually
carrying traffic.
...
**A revoked token fails on its next call, with no restart.** The door reads the
row per call rather than caching a verdict, so a revocation from the dashboard,
from another pod, or from a shell takes effect immediately everywhere. The row
survives revocation — it is the record that the credential existed, who minted
...
[the credential](#the-credential) already covers — mint one with
...
That is also why rotating this credential needs no rollout, unlike the others
