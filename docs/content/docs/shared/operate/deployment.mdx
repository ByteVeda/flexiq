...
manager, or language runtime. Use logs, health endpoints, or `kubectl debug`,
which runs its own tooling image as a separate ephemeral container sharing the
pod's namespaces and leaves the distroless target unchanged, instead of
expecting `docker exec ... sh` to work.
