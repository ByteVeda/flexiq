...
4. **Redis push-dispatch fallback window**: with `push_dispatch=True` on
   Redis, work nothing announces over the wake channel — a delayed job coming
   due that another process enqueued, a resumed queue, a freed rate/concurrency
   slot — waits on a 2s fallback timer rather than an immediate wake, longer
   than the ≤200ms polling ceiling it replaced.
...
4. **Redis push-dispatch fallback window**: with `pushDispatch: true` on
   Redis, work nothing announces over the wake channel — a delayed job coming
   due that another process enqueued, a resumed queue, a freed rate/concurrency
   slot — waits on a 2s fallback timer rather than an immediate wake, longer
   than the ≤200ms polling ceiling it replaced.
...
5. **Redis push-dispatch fallback window**: with `pushDispatch(true)` on
   Redis, work nothing announces over the wake channel — a delayed job coming
   due that another process enqueued, a resumed queue, a freed rate/concurrency
   slot — waits on a 2s fallback timer rather than an immediate wake, longer
   than the ≤200ms polling ceiling it replaced.

6. **Database performance**: slow dequeue queries. Check SQLite WAL size or
   Postgres query plans.
...
1. **Batch size too small**: unset, `SchedulerConfig::batch_size` defaults to
   `8` on Redis and `1` — one job per round trip — on SQLite/Postgres. Set it
   explicitly, and set `max_in_flight` to the worker's own parallelism so a
   deep backlog isn't claimed by one process and starved from its peers.
...
5. **Redis push-dispatch fallback window**: on Redis, push-dispatch is on by
   default, and work nothing announces over the wake channel — a delayed job
   coming due that another process enqueued, a resumed queue, a freed
   rate/concurrency slot — waits on a 2s fallback timer rather than an
   immediate wake, longer than the ≤200ms polling ceiling it replaced.
...
```json
{ "errtype": "TaskError", "message": "card declined", "traceback": [] }
```

`errtype` is always `"TaskError"` and `traceback` is always `[]` — Rust has no
exception type to name and no stack to attach, and `[]` is the contract's
no-frames case. The message is whatever
