...
4. **Redis push-dispatch fallback window**: on Redis, push-dispatch is on by
   default. Enqueues are announced, delayed ones included, and the worker's
   own retries and deferrals (rate limit, concurrency, circuit breaker) arm
   a local timer, so jobs usually dispatch on time. The rest — a resumed
   queue, a delayed job enqueued before the worker started, a deadline past
   what one large batch announces or the listener can buffer — falls back to
   a 2s timer rather than an immediate wake, longer than the ≤200ms polling
   ceiling it replaced. `push_dispatch=False` restores polling.
...
4. **Redis push-dispatch fallback window**: on Redis, push-dispatch is on by
   ...
   what one large batch announces or the listener can buffer — falls back to
   a 2s timer rather than an immediate wake, longer than the ≤200ms polling
   ceiling it replaced. `pushDispatch: false` restores polling.
...
5. **Redis push-dispatch fallback window**: on Redis, push-dispatch is on by
   ...
   what one large batch announces or the listener can buffer — falls back to
   a 2s timer rather than an immediate wake, longer than the ≤200ms polling
   ceiling it replaced. `.push_dispatch(false)` on the worker builder
   restores polling.
