...
4. **Redis push-dispatch fallback window**: on Redis, push-dispatch is on by
   default. Every enqueue, delayed ones included, and the worker's own
   retries and rate-limit deferrals are announced, so they dispatch on time.
   Work nothing announces — a resumed queue, a delayed job enqueued before
   the worker started — waits on a 2s fallback timer rather than an
   immediate wake, longer than the ≤200ms polling ceiling it replaced.
   `push_dispatch=False` restores polling.
...
4. **Redis push-dispatch fallback window**: on Redis, push-dispatch is on by
   ...
   the worker started — waits on a 2s fallback timer rather than an
   immediate wake, longer than the ≤200ms polling ceiling it replaced.
   `pushDispatch: false` restores polling.
...
5. **Redis push-dispatch fallback window**: on Redis, push-dispatch is on by
   ...
   the worker started — waits on a 2s fallback timer rather than an
   immediate wake, longer than the ≤200ms polling ceiling it replaced.
   `.push_dispatch(false)` on the worker builder restores polling.
