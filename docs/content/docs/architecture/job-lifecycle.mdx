...
there it either finishes as **Complete**, or the task fails and it goes back
...
<Callout type="info" title="complete vs completed">
  ...
  above). Aggregate views — the queue's stats call and the CLI's job summary —
  bucket finished jobs under the key `"completed"` instead. A job listing
  filtered by status accepts either spelling.
</Callout>
