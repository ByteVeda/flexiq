...
<SdkLink to="modules/mesh">Mesh Scheduling guide</SdkLink>.
...
The mesh is implemented as a standalone Rust crate (`crates/flexiq-mesh/`) with
no binding dependency. It is feature-gated behind the `mesh` cargo feature,
which each SDK's crate forwards. Depends on `flexiq-core` for the
...
<SdkLink to="modules/mesh#gossip-encryption">Gossip encryption</SdkLink> for setup.
...
The ring recalculates whenever a peer enters or leaves the alive set — a join,
a graceful leave, a suspicion, or a death. A peer that is merely gossiping fresh
load numbers does not move it. The `ring_recalculations` metric counts those
changes, so a fast climb means placements keep moving under you.
...
| `ring_recalculations` | Hash ring rebuilds — a peer entering or leaving the alive set |
...
- <SdkLink to="modules/mesh">Mesh Scheduling guide</SdkLink> — configuration, cluster topology, Docker Compose example
...
- [Failure Model](/architecture/failure-model) — how flexiq handles crashes and timeouts
