...
flexiq also supports PostgreSQL as an alternative storage backend. See the
<SdkLink to="operate/backends">Postgres backend guide</SdkLink> for full details.
...
- **Schema isolation**: all tables are created inside a configurable PostgreSQL schema
  (default: `flexiq`), with `search_path` set on each connection
