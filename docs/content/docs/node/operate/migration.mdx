...
description: "Concept mapping from BullMQ to the FlexiQ Node SDK."
---
...
| `QueueEvents` | `queue.on(event, …)` + [middleware](/node/guides/extend/middleware) |
...
| Flows (parent/child) | [Workflows](/node/modules/workflows) (DAG, fan-out, gates, saga) |
| `queue.getJobCounts()` | `await queue.stats()` / `statsAllQueues()` |
| Bull Board | the built-in [dashboard](/node/modules/dashboard) |
...
- **Backend choice** — start on SQLite with zero infrastructure; move to Postgres
  ...
  [backends](/node/operate/backends).
...
- **Built-in orchestration** — DAG [workflows](/node/modules/workflows) and a
  [mesh](/node/modules/mesh) replace hand-rolled flows and external
  schedulers.
...
<Callout type="info">
  Migrating incrementally? Point FlexiQ at the same Redis you already run, move
