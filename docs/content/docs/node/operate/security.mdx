...
msgpack](/node/guides/extend/serializers) and stored as plain bytes — by
...
- Don't put secrets (tokens, PII) directly in task args. Pass an id and fetch the
  ...
  [resource](/node/modules/injection/dependency-injection).
...
// timing-safe compare against the x-flexiq-signature header
```
...
```bash
FLEXIQ_WEBHOOKS_ALLOW_PRIVATE=1   # disable the guard for local development
```
...
The [dashboard](/node/modules/dashboard) runs in **open mode** by
...
auth via the [Express](/node/guides/extend/express#dashboard) /
[Fastify](/node/guides/extend/fastify) helpers.
...
- [ ] Webhook receivers verify the HMAC signature, and
      `FLEXIQ_WEBHOOKS_ALLOW_PRIVATE` is unset in production.
...
- [ ] Logs don't echo sensitive payloads (use `onEnqueue` to
      [redact](/node/modules/injection/interception)).
