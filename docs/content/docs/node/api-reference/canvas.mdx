...
examples, see the [Canvas guide](/node/guides/workflows/canvas).
...
[step option](/node/guides/workflows/building#step-options) except `after`,
...
<Callout type="info" title="The callback does not receive the group's results">
  ...
  aggregate results, use [`fanOut`/`fanIn`](/node/guides/workflows/fan-out)
...
[sub-workflow](/node/guides/workflows/sub-workflows)) and run a worker to
execute it.
