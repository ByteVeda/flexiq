...
The [symbol index](/node/api-reference/symbols/queue) lists the native queue's
spelling, which differs from the API for several pub/sub calls. The mapping:

| Declared | API |
|---|---|
| `readTopicMessages` | `readTopic()` |
| `ackTopicCursor` | `ackTopic()` |
| `leaseTopicMessages` | `leaseTopic()` — the declared `visibilityMs` is milliseconds; `opts.visibility` is seconds |
| `registerSubscription` | written by `subscriber()` via `declareSubscriptions()`, and by `runWorker()` at startup |
| `setSubscriptionActive` | `pauseSubscription()` / `resumeSubscription()` |
| `topicBacklogStats` | `topicStats()` |
