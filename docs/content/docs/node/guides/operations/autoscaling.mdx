...
`app` is the same module `taskito run` loads — one that exports a configured
...
```bash
taskito --db flexiq.db autoscale ./app.js --min-workers 2 --max-workers 20
```
...
ExecStart=/usr/bin/npx taskito --db /var/lib/taskito/flexiq.db autoscale ./app.js --min-workers 2 --max-workers 20
Restart=always
KillMode=process
TimeoutStopSec=120
...
```ts
import { Autoscaler } from "@byteveda/taskito";
...
On Kubernetes, run `serveScaler` (or `taskito scaler`) and let KEDA size the
