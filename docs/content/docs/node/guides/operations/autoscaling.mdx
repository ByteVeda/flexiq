...
ExecStart=/usr/bin/npx taskito --db /var/lib/taskito/taskito.db autoscale ./app.js --min-workers 2 --max-workers 20
Restart=always
KillMode=process
TimeoutStopSec=120
...
<Callout type="info">
  ...
  `TASKITO_LOG_LEVEL=info` (or `setLogLevel("info")`) to see the scaling
  history.
</Callout>
