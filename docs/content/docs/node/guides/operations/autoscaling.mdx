...
- **Scale-up** takes the *minimum* recent recommendation, so the pool grows as
  soon as any recent tick asked for capacity. `scaleUpWindowMs` defaults to `0`
  — immediate, because an extra worker for a few seconds costs far less than a
  backlog.
- **Scale-down** takes the *maximum* recent recommendation, so a brief lull
  can't tear the pool down. `scaleDownWindowMs` defaults to 5 minutes, matching
  the Kubernetes HPA's downscale stabilisation.
