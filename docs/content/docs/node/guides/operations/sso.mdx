...
<Mermaid
  chart={`sequenceDiagram
    ...
    D-->>B: 302 to next<br/>Set-Cookie: flexiq_session, taskito_csrf`}
/>
...
2. **Set env vars** before starting the dashboard:
   ...
   ```bash
   export FLEXIQ_DASHBOARD_OAUTH_REDIRECT_BASE_URL=https://taskito.your-company.com
   ...
   export FLEXIQ_DASHBOARD_OAUTH_GOOGLE_ALLOWED_DOMAINS=your-company.com
   ```
...
2. Env vars:
   ...
   export FLEXIQ_DASHBOARD_OAUTH_GITHUB_ALLOWED_ORGS=your-org,partner-org
   ```
...
export FLEXIQ_DASHBOARD_OAUTH_OIDC_MICROSOFT_LABEL="Microsoft 365"
```
...
1. **`FLEXIQ_DASHBOARD_OAUTH_ADMIN_EMAILS` match** — a case-insensitive match
...
```bash
export FLEXIQ_DASHBOARD_OAUTH_ADMIN_EMAILS=alice@your-company.com,bob@your-company.com
```

<Callout type="warning" title="OAuth without an admin list">
  If providers are configured but `FLEXIQ_DASHBOARD_OAUTH_ADMIN_EMAILS` is
...
```bash
export FLEXIQ_DASHBOARD_PASSWORD_AUTH_ENABLED=false
```
...
The callback sets the same `flexiq_session` + `taskito_csrf` cookies as
...
FLEXIQ_DASHBOARD_OAUTH_REDIRECT_BASE_URL=https://taskito.company.com
