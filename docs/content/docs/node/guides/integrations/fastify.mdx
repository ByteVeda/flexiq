...
Peer dependency: `fastify`. Import from the `@byteveda/taskito/contrib/fastify`
subpath.
...
`taskitoFastify` is a `FastifyPluginAsync`. It exposes the same twelve routes as
the [Express router](/node/guides/integrations/express#rest-routes) — including
