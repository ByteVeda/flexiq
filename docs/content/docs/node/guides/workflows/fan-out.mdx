...
> **Coming from BullMQ?** `FlowProducer` builds a **static** parent/child tree
> — `children` is a fixed array at definition time. flexiq's fan-out is
> **dynamic**: the child count comes from `list`'s return value at run time, so
> a batch of 3 files and a batch of 3,000 use the same workflow definition.
...
6. The `collect` fan-in node runs `summarize` with the results array as its single argument
