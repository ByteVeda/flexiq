...
```ts
import { NotesValidationError } from "@byteveda/taskito";
...
<Callout type="info">
  ...
  [`TaskitoError`](/node/api-reference/errors), so an enclosing
  `catch (e) { if (e instanceof TaskitoError) … }` catches it too.
</Callout>

## Dashboard rendering

The [dashboard](/node/guides/operations/dashboard) surfaces notes on the job
