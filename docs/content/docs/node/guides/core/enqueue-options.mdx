...
> flexiq's `uniqueKey` dedup is automatic and scoped to "while the first job
> is pending/running" — it frees up once that job finishes, whereas a BullMQ
> `jobId` stays taken until the finished job is removed.
