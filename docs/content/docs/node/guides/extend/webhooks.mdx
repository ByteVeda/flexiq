...
| `events` | Event names to deliver — any of the 29 dotted wire names (see [Events](/node/guides/extend/events) for the full list). Omit for all. |
...
[Events](/node/guides/extend/events) page instead of `OutcomeEvent`. A
...
guard](/node/operate/security#webhooks): loopback, private,
...
<Callout type="info">
  The [dashboard](/node/modules/dashboard) exposes `/api/webhooks` for
  managing hooks, and the same delivery log — paged and in snake_case — at
  [`/api/webhooks/:id/deliveries`](/node/modules/dashboard/rest-api).
