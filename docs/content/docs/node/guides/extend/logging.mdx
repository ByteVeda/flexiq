...
`createLogger(ns)` tags every line `[flexiq:ns]`; `child()` nests further:

```ts
const log = createLogger("worker"); // [flexiq:worker]
const poolLog = log.child("pool"); // [flexiq:worker:pool]
```
...
<Callout type="info">
  ...
  [progress](/node/guides/extend/monitoring), job
  [metadata](/node/guides/core/enqueue-options), or
  [events](/node/guides/extend/events).
</Callout>
