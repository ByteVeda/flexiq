...
```ts
import type { EventMap, EventPayload, WorkerEvent } from "@byteveda/flexiq";
```
...
<Callout type="info">
  ...
  context, error transformation) use [middleware](/node/guides/extend/middleware);
  to deliver events to external HTTP endpoints use
  [webhooks](/node/guides/extend/webhooks).
</Callout>
