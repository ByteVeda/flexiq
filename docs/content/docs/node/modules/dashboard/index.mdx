...
See the [REST API reference](/node/modules/dashboard/rest-api) for the full
endpoint list.
...
(or `flexiq dashboard --auth` from the CLI). With `authEnabled: true` the
...
[Express](/node/guides/extend/express#dashboard) or
