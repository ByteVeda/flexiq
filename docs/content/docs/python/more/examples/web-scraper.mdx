...
import json
import time
from flexiq import Queue, current_job, chain, group, chord

queue = Queue(
    db_path=".flexiq/scraper.db",
    workers=4,
    default_retry=3,
    default_timeout=60,
...
from flexiq import group, chord
...
if __name__ == "__main__":
    ...
    # "default" is required: scrape_page (run.py Option 1) runs on the default
    # queue. Omit the queues= argument to process every registered queue.
    queue.run_worker(queues=["default", "scraping", "processing", "storage"])
```

## Running it

<Tabs items={["Terminal 1 — Worker", "Terminal 2 — Enqueue", "Terminal 3 — Monitor"]}>
