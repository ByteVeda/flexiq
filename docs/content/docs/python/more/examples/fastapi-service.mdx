...
```python
"""FastAPI image processing service with flexiq."""
...
queue = Queue(db_path=".flexiq/images.db", workers=4, result_ttl=3600)
...
# Mount the flexiq router — adds /tasks/stats, /tasks/jobs/{id},
# /tasks/jobs/{id}/progress (SSE), and more.
...
<Tabs items={["Terminal 1 — Worker", "Terminal 2 — API Server", "Terminal 3 — Client"]}>
  <Tab value="Terminal 1 — Worker">

```bash
flexiq worker --app app:queue
```

  </Tab>
  <Tab value="Terminal 2 — API Server">
...
| Pattern | Where |
|---|---|
| FastAPI integration | `FlexiQRouter(queue)` mounted at `/tasks` |
...
| SSE streaming | `/tasks/jobs/{id}/progress` endpoint from `FlexiQRouter` |
