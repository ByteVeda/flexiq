...
claims the [deferred declarations](/python/guides/core/discovery) that import
leaves pending — so an app that imports its task modules *below* the `Queue`
needs no `autodiscover` call for the CLI to see them.
