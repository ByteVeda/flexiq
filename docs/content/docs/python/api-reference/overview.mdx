...
| [Worker](/python/api-reference/worker) | `run_worker()` — pools, tags, shutdown, worker introspection |
...
| [CLI](/python/api-reference/cli) | `flexiq` command-line interface — `worker`, `dashboard`, `info`, `pause`, `resume`, `scaler`, `autoscale`, `resources`, `reload` |
