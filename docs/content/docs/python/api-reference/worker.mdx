...
| `mesh` | `MeshWorker \| None` | `None` | Gossip-based discovery, affinity, and work stealing. Requires the `mesh` cargo feature. |
...
<Callout type="warning">
  Set `drain_timeout` above your longest task's `timeout`, or shutdown will cut
  off jobs that would have finished. A job interrupted by the drain deadline is
  retried according to its budget, so the work is not lost — but it is repeated.
</Callout>
...
taskito worker --app myapp:queue --queues emails,default --pool prefork
taskito worker --app myapp:queue --drain-timeout 60
```
