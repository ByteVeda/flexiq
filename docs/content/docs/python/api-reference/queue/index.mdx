---
title: Queue
description: "The central class for creating and managing a task queue."
---
...
<Callout type="info" title="Sub-pages">
  ...
  - **[Pub/Sub](/python/api-reference/queue/pubsub)** — topic subscriptions, fan-out publish
</Callout>
...
| `interception` | `str` | `"off"` | Argument interception mode: `"strict"`, `"lenient"`, or `"off"`. See [Resource System](/python/modules/injection). |
...
| `inject` | `list[str] \| None` | `None` | Resource names to inject as keyword arguments. See [Resource System](/python/modules/injection). |
...
| `compensates` | `TaskWrapper \| str \| None` | `None` | Saga compensator for this task, enqueued in reverse order if a later workflow step fails. See [Sagas](/python/modules/workflows/saga). |
...
<Callout type="info" title="More task() parameters">
  `predicate_extras` and `default_defer_seconds` fine-tune `predicate`/`on_false` —
  see [Predicates](/python/guides/core/predicates). Per-call idempotency overrides
...
```python
queue.autodiscover(package: str = "tasks") -> list[str]
```

Import `package` and every module beneath it, then register every task declared
with the queue-less `@flexiq.task()` decorator. Returns the sorted names now
registered on this queue.

| Parameter | Type | Default | Description |
|---|---|---|---|
| `package` | `str` | `"tasks"` | Dotted path to the package (or single module) holding the declarations. |

```python
from flexiq import Queue

queue = Queue(db_path="app.db")
queue.autodiscover("myapp.tasks")  # ['myapp.tasks.invoices.send_invoice', ...]
```

Idempotent — a second call returns the same list rather than an empty one, and a
second `Queue` in the same process receives the same tasks. The registry is also
drained at `Queue(...)` construction and at `run_worker()`, so a worker
entrypoint that imports its task modules directly never has to call this.

Raises `ImportError` if `package` or anything beneath it fails to import (never
swallowed), and [`DuplicateTaskError`](/python/api-reference/errors) if a
discovered task claims a name this queue already registered for a different
function. See [Task discovery](/python/guides/core/discovery).
...
Schedules reach storage when `run_worker()` starts — that is where the decorator
turns into a `register_periodic` write, so a producer that never runs a worker
schedules nothing. These act on what is in storage, so they outlive the
decorator that declared them: deleting a schedule the decorator still declares
means it comes back on the next worker start.
...
| `notes` | `dict \| None` | `None` | Structured annotations, max **15** top-level keys. See [Structured notes](/python/guides/extend/notes). |
| `depends_on` | `str \| list[str] \| None` | `None` | Job ID(s) this job depends on. See [Dependencies](/python/guides/core). |
...
Enqueue multiple jobs in a single transaction for high throughput. Supports both
uniform parameters (applied to all jobs) and per-job lists.

One transaction, so it is all-or-nothing: a rejected entry fails the whole call
rather than leaving part of the batch enqueued. Declared as `enqueue_batch`,
which takes parallel lists — the shell fans one task name and one args list out
into them. Interceptors and enqueue gates run per entry, before the batch is
sent, and a queue's `max_pending` cap is checked against the batch size so a
batch is rejected whole rather than overshooting by its own length.
...
| `notes` | `dict \| None` | `None` | Uniform [structured notes](/python/guides/extend/notes) for all jobs (≤ 15 keys) |
