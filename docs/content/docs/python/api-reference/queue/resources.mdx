...
Both context managers yield a `DistributedLock`. Drive it directly when the
critical section does not fit a `with` block — a callback, or an acquire in one
method and a release in another. **Declared** names the `PyQueue` primitive each
member calls; that is the spelling the
