...
<Callout type="warn" title="Auth-gated only when authentication is enabled">
  ...
  [Dashboard Authentication](/python/modules/dashboard/authentication) for
...
are also public — see [SSO](/python/modules/dashboard/sso#api-surface) for
the full flow.
...
Full guide: [Webhooks](/python/guides/extend/webhooks).
...
Full guide: [Task & Queue Overrides](/python/modules/dashboard/task-overrides).
...
Public readiness check. `ready` (HTTP `200`) when storage answered, the worker
registry could be read, and no advertised worker resource is unhealthy;
`degraded` (HTTP `503`, with the same body) when any of those checks failed, so
an orchestrator drains the instance.

The worker count is reported but never folded into the status: with no worker
running, `checks.workers` is `{"count": 0, "status": "none"}` and the response
is still `ready`. Readiness answers whether this process can serve the queue
API, and workers are a separate deployment that may legitimately scale to zero —
so a green `/readiness` does not mean jobs are being consumed. Alert on the
fleet size as its own signal (`queue.workers()`, or the dashboard Workers
panel).
...
The retention windows a worker is applying to this queue's namespace — what
the Settings page echoes to explain why rows leave the listings.
...
### `GET /api/retention/dry-run`

How many rows a purge would delete right now, without deleting — the Settings
