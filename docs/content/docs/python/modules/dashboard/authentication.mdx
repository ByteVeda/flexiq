...
```bash
flexiq dashboard --app myapp:queue --auth
```
...
```bash
python manage.py flexiq_dashboard --auth
```

Or set `FLEXIQ_DASHBOARD_AUTH = True` in `settings.py` to make it the
...
- **Sessions** are stored server-side under
  ...
  `flexiq dashboard --insecure-cookies`).
- **CSRF** uses the double-submit pattern: a non-HttpOnly cookie named
  `flexiq_csrf` carries a per-session token that the SPA reads and
...
2. For every read: send the `flexiq_session` cookie
3. For every write: send the `flexiq_session` cookie + `flexiq_csrf`
...
Set `FLEXIQ_WEBHOOKS_ALLOW_PRIVATE=1` to disable the guard for local
...
password auth — see [SSO (OAuth & OIDC)](/python/modules/dashboard/sso).
