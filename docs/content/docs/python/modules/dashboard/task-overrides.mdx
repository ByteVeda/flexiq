...
Plus a separate but related toggle for
[**middleware**](/python/guides/extend/middleware) on a per-task
...
This split is intentional. Queue-level pause is a fast-path safety
valve wired directly into the scheduler's poll loop; retry/rate-limit
changes need scheduler buy-in and are deliberately "restart to apply"
so operators have a clear mental model of when the new values take
over.

<Callout type="info" title="Per-task pause and hot-reload are planned follow-ups">
...
in the `dashboard_settings` table — for a `Queue` with a `namespace`,
under `overrides:ns:<len>:<namespace>:task:<task_name>` instead, so
tenants sharing a database never share an override. SQLite, PostgreSQL, and Redis
...
```python
queue.pause("email")
# ... or from the dashboard: Infrastructure → Queues → email → Pause
```

Existing in-flight jobs finish normally; nothing new dequeues from
...
- [Dashboard REST API: Tasks & overrides](/python/modules/dashboard/rest-api#tasks-and-overrides)
- [Dashboard REST API: Middleware](/python/modules/dashboard/rest-api#middleware)
