...
flexiq ships with a built-in web dashboard for monitoring jobs, inspecting
...
> Coming from Celery? This is flexiq's built-in equivalent of Flower — no
...
flexiq dashboard --app myapp:queue --host 0.0.0.0 --port 9000
```
...
<Callout type="warn" title="The dashboard serves openly by default">
  ...
  [Authentication](/python/modules/dashboard/authentication) for the full
...
| Configuration | **Tasks** | Decorator defaults + runtime overrides per task ([guide](/python/modules/dashboard/task-overrides)) |
| Configuration | **Webhooks** | HTTP event subscriptions with delivery history + replay ([guide](/python/guides/extend/webhooks)) |
| Configuration | **Settings** | Dashboard branding, external links, integrations |

The full REST API surface is documented at
[Dashboard REST API](/python/modules/dashboard/rest-api).
...
See [Authentication](/python/modules/dashboard/authentication) for the env
...
[Webhooks](/python/guides/extend/webhooks).
...
[Task & Queue Overrides](/python/modules/dashboard/task-overrides).
...
The **Registry** column is a fingerprint of the tasks each worker has handlers
for. Workers running the same code share one value; a worker whose value the
rest of the fleet does not share is badged, because a job for a task only some
workers know fails wherever it lands rather than waiting for one that can run
it. A worker whose SDK does not report a registry shows no value.
