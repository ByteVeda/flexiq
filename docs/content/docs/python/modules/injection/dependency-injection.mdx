...
FlexiQ runs the async factory on the worker's event loop before accepting
tasks.
...
| `frozen` | `False` | Wrap instance in a read-only wrapper (see [Configuration](/python/modules/injection/configuration#frozen-resources)). |
...
[Configuration](/python/modules/injection/configuration) for details.
...
Resources can declare other resources they depend on. FlexiQ resolves the
...
Teardown callables can be async — FlexiQ awaits them if they return a
coroutine.
...
[Observability](/python/modules/injection/observability) for details.
