...
[worker resource](/python/modules/injection/dependency-injection), like a
...
```bash
pip install flexiq[aws]   # adds boto3>=1.20
```
...
```bash
pip install flexiq[gcs]   # adds google-cloud-storage>=2.0
```
...
Use `NoProxy` when the serializer can handle the value directly — flexiq's
...
[Pluggable Serializers](/python/guides/extend/serializers) for the
...
<Callout type="warning">
  ...
  worker-owned [resource](/python/modules/injection/dependency-injection)
  instead.
</Callout>
...
See [Observability](/python/modules/injection/observability) for Prometheus
