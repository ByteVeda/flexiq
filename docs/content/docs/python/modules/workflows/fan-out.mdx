...
2. The tracker splits the returned list into individual items, one per child
3. It creates N child nodes and N jobs, all ready to run immediately (no `depends_on` wait)
4. Each child runs independently in parallel
5. When every child has completed, the tracker marks the fan-out parent done
...
Combine with [conditions](/python/modules/workflows/conditions) for more
control:
