...
description: "Automatically scale flexiq worker processes on bare metal, Docker, or systemd — no Kubernetes required."
---
...
use [KEDA + Kubernetes](/python/modules/autoscaling/keda).
...
<Callout type="info">
  The autoscaler logs to `flexiq.autoscale` at INFO level. Each tick
...
For Kubernetes deployments, use [KEDA](/python/modules/autoscaling/keda).
