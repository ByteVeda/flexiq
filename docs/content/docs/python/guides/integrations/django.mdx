...
flexiq provides Django admin views for browsing jobs, inspecting dead
...
This adds flexiq views to the default `admin.site`.
...
admin_site = FlexiQAdminSite(name="flexiq_admin")
```
...
Create a `flexiq` queue instance in your Django project — this is the
