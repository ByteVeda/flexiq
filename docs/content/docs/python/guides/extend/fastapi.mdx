...
description: "FlexiQRouter — pre-built APIRouter with job status, SSE progress, dead letter management."
---
...
| `GET` | `/readiness` | Readiness check — `503` when degraded |
...
The stream sends a JSON event every 0.5 seconds while the job is active,
then a final event when the job reaches a terminal state.

## Pydantic response models

All endpoints return validated Pydantic models with clean OpenAPI docs.
You can import them for type-safe client code:
