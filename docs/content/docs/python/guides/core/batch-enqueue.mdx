...
Insert many jobs in a single database transaction (SQLite or Postgres) for high
throughput.

<Callout type="info" title="Not the same as task batching">
  *Batch enqueue* (`task.map()` / `enqueue_many()`) writes **many** separate jobs
  at once — one job per item. [*Task batching*](/python/guides/core/batching)
