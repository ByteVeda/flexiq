...
<Callout type="info" title="Not the same as batch enqueue">
  Task batching coalesces many `.delay()` calls into **one** job.
  [Batch enqueue](/python/guides/core/batch-enqueue)
...
Items that have been added to the in-memory accumulator but not yet flushed are **lost** if the process crashes. flexiq does **not** persist them between the `.delay()` call and the flush.
...
See also the [`canvas.chunks()`](/python/modules/workflows/canvas) primitive — it's a producer-side equivalent that creates `N` separate jobs each holding a sub-list, with full per-chunk retry semantics.
