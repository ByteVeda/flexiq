...
taskito uses a **token bucket** (a standard rate-limit algorithm that refills allowance over time) algorithm to limit how fast tasks execute.
...
> effective rate scales with worker count. taskito enforces it **globally per task
