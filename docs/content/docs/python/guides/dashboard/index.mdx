...
> Coming from Celery? This is taskito's built-in equivalent of Flower — no
...
hash-busted multi-file assets under `sdks/python/taskito/static/dashboard/`.
...
<Callout type="info" title="Zero extra dependencies at runtime">
  ...
  `sdks/python/taskito/static/dashboard/` and is served by the Python
...
under `sdks/python/taskito/static/dashboard/`. The built assets aren't
