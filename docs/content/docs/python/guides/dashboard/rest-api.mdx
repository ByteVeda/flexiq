...
Public readiness check. `ready` (HTTP `200`) when storage answered and no
advertised worker resource is unhealthy; `degraded` (HTTP `503`, with the same
body) when one of those checks failed, so an orchestrator drains the instance.
