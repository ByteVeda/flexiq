...
<Mermaid
  chart={`sequenceDiagram
    ...
    D-->>B: 302 to /<br/>Set-Cookie: flexiq_session, taskito_csrf`}
/>
...
1. **`FLEXIQ_DASHBOARD_OAUTH_ADMIN_EMAILS` match** — case-insensitive
...
The callback uses the same `flexiq_session` + `taskito_csrf` cookies
