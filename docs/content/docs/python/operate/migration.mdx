...
description: "Side-by-side concept mapping and step-by-step migration from Celery to flexiq."
---

import { Callout } from "fumadocs-ui/components/callout";
import { Tab, Tabs } from "fumadocs-ui/components/tabs";

This guide maps Celery concepts to their flexiq equivalents. If you're
...
<Callout type="info">
  flexiq uses 6-field cron expressions (with seconds). Celery's `crontab()`
...
```bash
flexiq worker --app myapp:queue --queues emails,default
```
...
flexiq's test mode uses a context manager instead of a global setting, so
it's safe to use in parallel test runs.

## What flexiq doesn't have
...
- [ ] Change `celery worker` to `flexiq worker` in deployment scripts
