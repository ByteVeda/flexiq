...
controls how many jobs the scheduler claims per round-trip. Left unset, it
defaults to `1` on SQLite/Postgres (preserving pre-0.15 behaviour exactly) and
`8` on Redis.
...
**Ships in the published wheel; on by default for Redis.** No custom build is
needed. A Redis-backed `Queue` gets event-driven dispatch automatically; pass
`push_dispatch=False` to opt back out to polling. SQLite and Postgres keep
polling unless you pass `push_dispatch=True`.
...
| Redis | Fully event-driven via pub/sub `SUBSCRIBE` |
| Postgres | Falls back to a faster poll — native `LISTEN` listener pending |

Polling remains the default on SQLite/Postgres, and the safety-net fallback
everywhere. There is no correctness difference between the two modes: a missed
wake is only ever caught by a 2s fallback timer rather than an immediate one,
never lost.

<Callout type="info">
  Redis's wake channel is scoped by key prefix, namespace and queue, not by
  database number. Two deployments on the same server with the same namespace
  but different `SELECT`ed databases wake each other's schedulers. Nothing
  crosses over — a woken scheduler reads only its own database and finds
  nothing to claim — so the cost is a spurious empty dequeue. A distinct
  `namespace=` per deployment avoids it; this SDK has no prefix override.
</Callout>
...
## 6. Job payloads side table (`job_payloads`) — superseded

**Superseded:** 0.15 briefly dual-wrote payload BLOBs to a separate
...
- [ ] **Batch dequeue:** decide on a `scheduler_batch_size` for your workload.
  Unset is backend-chosen (1 on SQLite/Postgres, 8 on Redis) and safe; raise it
  if you regularly see a backlog.
- [ ] **Push dispatch:** on by default for Redis; pass `push_dispatch=True` to
  opt in on SQLite/Postgres too, or `push_dispatch=False` to opt out on Redis.
