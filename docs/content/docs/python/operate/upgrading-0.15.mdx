...
<Callout type="info">
  Redis's wake channel is scoped by key prefix, not by database number. Two
  deployments on the same server and the same prefix but different
  `SELECT`ed databases will cross-wake each other — harmless (a spurious
  empty dequeue), but this SDK has no prefix override today, so isolating two
  deployments means separate Redis servers, not just separate databases.
</Callout>
