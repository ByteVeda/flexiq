...
flexiq is a library you embed in your own services, so most of its security
...
<Callout type="warn" title="Storage write access ≈ code execution">
  flexiq trusts its backing store. The default serializer can execute code
...
See [Pluggable Serializers](/python/guides/extend/serializers) for the full
matrix.
...
- **Cookies.** Session and CSRF cookies carry `HttpOnly` (session),
  ...
  or `flexiq dashboard --insecure-cookies` — only on a trusted network.
...
<Callout type="warn" title="Contrib routers are not auth-gated by default">
  The FastAPI/Flask routers in `flexiq.contrib` expose job-control endpoints
...
- **Recipe signing.** Set `recipe_signing_key=` (or `FLEXIQ_RECIPE_SECRET`)
...
- [ ] Set `FLEXIQ_DASHBOARD_METRICS_TOKEN` if metrics are network-reachable.
...
- [ ] Do **not** set `FLEXIQ_WEBHOOKS_ALLOW_PRIVATE` in production.
