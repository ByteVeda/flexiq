...
see [Backup and restore](/python/operate/backup) for what WAL means in the
SQLite backend; Postgres uses WAL primarily for crash recovery and replication
rather than reader/writer concurrency)
