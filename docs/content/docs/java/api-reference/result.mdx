...
`Optional` is "no value to read", never "the job succeeded with nothing". When
the difference matters, unwrap the job snapshot and read its status —
`getJob(id).map(job -> job.status)`.
