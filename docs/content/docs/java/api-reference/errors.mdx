...
| `DuplicateTaskException` | `FlexiQException` | Two handlers claim one task name — two providers on the classpath, or `discover()` landing on a name already registered on the builder. See [Task discovery](/java/guides/core/discovery). |
