...
The job running on **this thread**, so a handler reaches its own identity
without threading it through every signature. A `static` accessor: it is reached
through the type, because a handler has no `JobContext` until this hands it one.
