...
`FlexiQEvent` is a sealed interface with one method, `EventName name()`.
...
<Callout type="info">
  ...
  [middleware](/java/guides/extend/middleware); to deliver events to
  external HTTP endpoints use
  [webhooks](/java/guides/extend/webhooks).
</Callout>
