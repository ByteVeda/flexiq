...
rejects a malformed spec rather than running unthrottled — a count below `1`
counts as malformed, since a bucket that never holds one token never releases a
job, and so does a count that is not finite:
