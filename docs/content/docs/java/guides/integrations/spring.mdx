...
```kotlin
implementation("org.byteveda:flexiq-spring:0.23.0")
```
...
| `flexiq.dashboard.auth-enabled` | `boolean` | `false` | Enable [session auth](/java/guides/operations/dashboard#session-auth-opt-in) (login/setup, CSRF, roles); off means the dashboard serves openly |
