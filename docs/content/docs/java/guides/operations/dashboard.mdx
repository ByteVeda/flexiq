...
| `GET /readiness` | Gated when auth is on (see below) | Storage and worker-resource readiness; `503` when degraded — the [worker count never degrades it](/java/guides/observability/monitoring#health--readiness) |
