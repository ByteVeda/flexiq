...
MessagePack](/java/guides/extend/serializers) and stored as plain bytes
...
- Don't put secrets (tokens, PII) directly in task payloads. Pass an id and
  ...
  [resource](/java/modules/injection).
...
[Webhooks: SSRF guard](/java/guides/extend/webhooks#ssrf-guard).
...
The [dashboard](/java/modules/dashboard) serves **openly by
...
[OAuth/OIDC](/java/modules/dashboard/sso). Passing a `token` instead switches
...
- [ ] Logs don't echo sensitive payloads (redact in `onEnqueue`
      [middleware](/java/guides/extend/middleware)).
