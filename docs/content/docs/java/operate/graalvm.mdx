...
description: "Compile flexiq applications to a native binary with GraalVM."
---
...
- **No runtime reflection for handlers** — the `flexiq-processor` annotation
...
- **Shipped reachability metadata** — the `flexiq` jar carries its own
...
the JVM, so the [deployment](/java/operate/deployment) properties
...
<Callout type="info">
  ...
  a task, a worker, an event listener, a `@TaskHandler` the worker picks up
  through `discover()`, periodic registration, and dead-letter listing,
  compiled and executed as a native image in CI.
</Callout>
