...
dependencies {
    ...
    runtimeOnly("org.byteveda:taskito:0.23.0:${osdetector.classifier}") // native library
...
the native library from source and pointing `-Dtaskito.native.lib` at it.
...
Taskito.builder().sqlite("flexiq.db").open();                   // SQLite (default)
