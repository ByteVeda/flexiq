...
The Java SDK is a typed shell over the FlexiQ Rust core, bound through a
...
dependencies {
    ...
    runtimeOnly("org.byteveda:flexiq:2.0.0:${osdetector.classifier}") // native library
...
The `flexiq-processor` artifact is optional but recommended: it turns
...
FlexiQ.builder().redis("redis://localhost").open();
```

`FlexiQ.builder().open()` with no backend configured uses SQLite at
