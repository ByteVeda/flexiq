...
taskito collapses all of that into the `Taskito` client: one SQLite file (or a
...
`Taskito.builder().sqlite(...).open()` is the whole setup.

<Callout type="info" title="Coming from a brokered queue or Spring @Async?">
  Your `ConnectionFactory` / `spring.rabbitmq.*` / JMS listener config
  disappears — a single `Taskito.builder().sqlite("flexiq.db").open()`
...
A failing job retries with **backoff** (each wait longer than the last) until
its budget is exhausted, then lands in the **dead-letter queue** (DLQ, a
holding area for exhausted jobs) — see
[dead-letter](/java/guides/reliability/dead-letter).
