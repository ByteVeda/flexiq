...
<Tabs items={["Programmatic", "FlexiQ.dashboard()", "CLI"]}>
...
flexiq.dashboard(8080, System.getenv("DASH_TOKEN"));
```

`FlexiQ.dashboard(port)` / `dashboard(port, authEnabled)` /
...
<Callout type="info">
  ...
  [Spring Boot: Dashboard auto-configuration](/java/guides/extend/spring#dashboard-auto-configuration).
</Callout>
...
(and optionally [OAuth/OIDC](/java/modules/dashboard/sso)) with server-side
...
```java
DashboardServer.start(flexiq, 8080, System.getenv("DASH_TOKEN"));
```
...
| `GET /readiness` | Gated when auth is on (see below) | Storage and worker-resource readiness; `503` when degraded — the [worker count never degrades it](/java/guides/extend/monitoring#health--readiness) |
...
| Group | Routes |
|---|---|
| Auth | `auth/status`, `auth/setup`, `auth/login`, `auth/logout`, `auth/whoami`, `auth/change-password`, `auth/providers`, `auth/oauth/start/{slot}`, `auth/oauth/callback/{slot}` — see [SSO](/java/modules/dashboard/sso) |
...
| Webhooks | `webhooks` (+ `/{id}`, `/{id}/test`, `/{id}/rotate-secret`, `/{id}/deliveries`, `/{id}/deliveries/{deliveryId}`, `/{id}/deliveries/{deliveryId}/replay`) — see [Webhooks: Dashboard management](/java/guides/extend/webhooks#dashboard-management) |
