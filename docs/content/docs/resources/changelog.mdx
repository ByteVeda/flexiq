...
- **A readiness probe that can actually run.** `/readiness` is gated alongside `/metrics`, so a
  ...
  `FLEXIQ_DASHBOARD_PUBLIC_READINESS=1` answers that one route without a credential; `/metrics`
...
- **Dashboard authorization is enforced server-side.** Mutating API routes now
  ...
  `FLEXIQ_DASHBOARD_METRICS_TOKEN`.
...
- **Django settings** -- `FLEXIQ_AUTODISCOVER_MODULE`, `FLEXIQ_ADMIN_PER_PAGE`, `FLEXIQ_ADMIN_TITLE`, `FLEXIQ_ADMIN_HEADER`, `FLEXIQ_DASHBOARD_HOST`, `FLEXIQ_DASHBOARD_PORT` control autodiscovery, admin pagination, branding, and dashboard bind address
...
- **Proxy security** -- HMAC-SHA256 recipe signing via `recipe_signing_key=` on `Queue()` or `FLEXIQ_RECIPE_SECRET` env var; reconstruction timeout via `max_reconstruction_timeout=`; file path allowlist via `file_path_allowlist=`; per-handler opt-out via `disabled_proxies=`
...
- **Django integration** — `FLEXIQ_BACKEND`, `FLEXIQ_DB_URL`, `FLEXIQ_SCHEMA` settings for configuring the backend from Django projects
...
- Default `db_path` now uses `.flexiq/` directory, with automatic directory creation
