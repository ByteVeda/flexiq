...
FlexiQ ships SDKs for three languages. This page is for everything else — Ruby,
C#, PHP, Elixir, anything with a gRPC library — talking to the
[producer door](/server) of a running
...
<Callout title="Go has a client already">
  You do not have to generate one:
  [`github.com/ByteVeda/flexiq/sdks/go/v2`](https://github.com/ByteVeda/flexiq/tree/master/sdks/go)
  implements this page — enqueue, batch, read, cancel, count, and submit and
  read a workflow — with the payload envelope, the reason-based errors and the
  bearer credential already handled.
  It is a producer client, not an SDK: it cannot execute tasks.
</Callout>
...
different door: [Custom executors](/server/custom-executors).
...
shows what those two answer with, and the one thing the credential does not
buy you.

**The release asset** is attached to every `server-v*` release from 2.0.0 — the
release the gRPC role shipped in, and so the first one with a wire contract to
publish:
...
`wire-vectors.json`, which is the authority for the payload envelope below, and
`REMOTE_SDK_CONTRACT.md`, which is the normative version of this page: the RPCs
a client must implement against the ones it may skip, stated as obligations
...
Either way you get `flexiq.v1.ProducerService` with nine RPCs: `Enqueue`,
`EnqueueBatch`, `GetJob`, `ListJobs`, `CancelJob`, `QueueStats`,
`SubmitWorkflow`, `GetWorkflowRun` and the server stream `WatchJobs`.

With neither, generate against the JSON facade instead:
[`contracts/openapi.json`](https://github.com/ByteVeda/flexiq/blob/master/contracts/openapi.json)
describes the eight unary RPCs as HTTP paths, in OpenAPI 3.1. `WatchJobs` has no
HTTP path, because a stream has no request/response mapping. It is generated
from the `.proto` files above and gated against them, so it is the same contract
read through a different generator ecosystem — not a second one. What it costs
is the payload envelope: see
...
are covered in
[API tokens](/server/operate/tokens).
...
Three rules your encoder must follow, all because the SDKs' automatic
...
- **64-bit finite floats only** — `fb`, never `f9` or `fa`, even where a narrower
  width would round-trip the value exactly. `1.5` is
  `fb 3f f8 00 00 00 00 00 00`, not `f9 3e 00`. If your language has a 32-bit
  float type, widen it before it reaches the encoder.

Readers accept every form a writer must not use, so a payload written before
these rules still runs. A non-finite float is the one stated exemption — CBOR
libraries hard-code RFC 8949's two-byte spelling of an infinity and a NaN, so its
width is not pinnable and a payload carrying one needs `unique_key` rather than
an `auto:` key.
...
[the full treatment](/server/operate/grpc#two-doors-for-one-payload-and-only-one-of-them-is-lossless).
...
| `GetJob`, `ListJobs`, `QueueStats`, `GetWorkflowRun`, `WatchJobs` | Yes — they have no side effects. |
...
in
[The same door, without gRPC](/server/operate/grpc#the-same-door-without-grpc),
and
...
| Cost | Mitigation |
|---|---|
| No task registration, middleware or worker — this is a producer, not an SDK | Run the workers in a language that has an SDK, or write a [custom executor](/server/custom-executors) |
...
| Completion arrives on a stream, not a callback | Watch the job with `WatchJobs` and read the result with `GetJob` once it is terminal. See [Watching jobs](/server/operate/grpc#watching-jobs) |
