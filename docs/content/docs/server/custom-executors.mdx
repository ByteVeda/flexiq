...
| Push | The request is abandoned, the attempt settles `Cancelled`, the target keeps running and its side effects still happen, and its result is fenced out on arrival. Unchanged for an accepted dispatch: the attempt is given up, and the target's later `Settle` is refused on the fence. |

An executor built against this page — attach, over `flexiq.executor.v1` or the
socket protocol either one — is the middle row: cancellation is real, because
there is a connection to send it down. [Push dispatch](/server/operate/push)
has no such connection; the scheduler can stop waiting on a job, but it cannot
reach into the target's process and stop it.
[Issue #846](https://github.com/ByteVeda/flexiq/issues/846) is the tracked
follow-up that changes the push row — until it lands, that gap is a known one,
not an oversight.
