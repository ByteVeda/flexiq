...
<Callout title="Go has an executor already">
  ...
  the lease echo, reconnect and rotation — behind a handler you register per
  task. Durable steps are the one thing it leaves out, so it advertises no
  `steps` capability and the scheduler sends it no step frames.
</Callout>
