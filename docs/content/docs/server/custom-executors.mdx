...
| Push, accepted (`202`) | The attempt settles `Cancelled`. The target's next `ExtendLease`, `ReportProgress`, `WriteTaskLog` or `Settle` is refused with reason `JOB_CANCELLED` — a target that polls stops then; one that never does runs to the end. |
