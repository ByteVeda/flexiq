...
[Clients without an SDK](/server/clients) does for
...
The same tarball carries
[`REMOTE_SDK_CONTRACT.md`](https://github.com/ByteVeda/flexiq/blob/master/contracts/REMOTE_SDK_CONTRACT.md),
which states this door's obligations normatively — which frames are mandatory,
which are gated on a capability, and what a lease is required on.

<Callout title="Go has an executor already">
  You do not have to write one:
  [`github.com/ByteVeda/flexiq/sdks/go/v2/executor`](https://github.com/ByteVeda/flexiq/tree/master/sdks/go/executor)
  implements this page — the handshake, the session token, the settling frames,
  the lease echo, durable steps, reconnect and rotation — behind a handler you
  register per task. It advertises all three capabilities below.
</Callout>
...
`cancel` above is this door's frame, and what it does for you follows the same
rule an attached executor gets — the scheduler sends it and you stop. That is
not true everywhere FlexiQ dispatches a job, and the difference is worth
knowing before you build against one topology and assume it generalizes:

| Topology | What `cancel()` promises |
|---|---|
| Native | The handler observes the storage flag and stops. |
| Attach | Within about a second the scheduler sends a `cancel` frame; the executor stops. |
| Push, in the request | The request is abandoned and the attempt settles `Cancelled`. The target sees only its connection close; its side effects still happen and its answer is fenced out. |
| Push, accepted (`202`) | The attempt settles `Cancelled`. The target's next `ExtendLease`, `ReportProgress`, `WriteTaskLog` or `Settle` is refused with reason `JOB_CANCELLED` by the replica that dispatched it, while it still remembers the ending — a target that polls stops then; one that never does runs to the end. Any `FAILED_PRECONDITION` here means stop. |

An executor built against this page — attach, over `flexiq.executor.v1` or the
socket protocol either one — is the attach row: cancellation stops the work,
because there is a connection to send it down. [Push dispatch](/server/operate/push)
has no such connection, so there cancel stops the *result* and tells a target
that asks. Moving a task from one topology to the other changes which promise
it gets.

A cancel reaches the scheduler through storage, so it does not matter where it
was made — the producer door's `CancelJob`, the dashboard, or an SDK sharing
the database all work the same way.

## The reporting RPCs, for work that outlived its request

`Attach` is not the only way into this door. A [push target](/server/operate/push)
never attaches — the scheduler dials *out* to it — but a job that cannot finish
inside the platform's request deadline still has to report somehow, so four
unary RPCs exist for exactly that:

| RPC | What it does |
|---|---|
| `Settle` | Reports the outcome of a dispatch the target answered `202 Accepted` to. Single-use for the attempt it names. |
| `ExtendLease` | Asks for longer before the attempt is considered lost. Repeatable; clamped to an hour per call. |
| `ReportProgress` | The `progress` frame, off the stream. |
| `WriteTaskLog` | The `task_log` frame, off the stream. |

They carry the **same frames** the stream does — a `SuccessFrame` reported over
`Settle` and one sent over `Attach` say the same thing — and they are fenced on
the same lease. `Settle` has no `slept` arm: a push dispatch has no step
session to resume.

On a push deployment the door serves only these four. `Attach` and `Heartbeat`
answer `FAILED_PRECONDITION` there, because there is nothing to attach *to* —
the service surface stays constant and the refusal says which deployment you
reached, rather than making "does this RPC exist" a configuration question.
...
[Clients without an SDK](/server/clients#the-payload-envelope).
