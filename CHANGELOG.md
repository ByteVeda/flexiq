...
process that has no FlexiQ binding at all. Everything here is additive — no public API was
removed or renamed in any SDK — but seven migrations run on first start and one of them changes
how `unique_key` deduplicates, so read [Upgrading](#upgrading-to-110) first.

### Added

- **Durable steps.** `ctx.step.run(key, fn)` and `ctx.step.sleep(key, duration)` in all three
  SDKs, backed by a `job_steps` table. A retried job replays the steps that already committed
  instead of re-running them, which is what makes a task that charges a card safe to retry.
...
- **`GET /metrics` is served on the gRPC listener**, so a release that enables only the gRPC
  ...
  and latencies.
- **`Storage` gained `enqueue_unique_reporting` and `enqueue_unique_batch_reporting`**, which
  return whether the enqueue deduplicated — the producer door has to answer that on the wire,
  and it is not computable above the backend. The existing names stay as wrappers. Only direct
  users of the `flexiq-core` crate are affected.
