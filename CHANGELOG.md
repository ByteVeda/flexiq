...
- **Kafka and NATS event sinks** (#971). Two more `kind`s for the event egress document.
  ...
  SASL PLAIN or SCRAM when configured; its client is pure Rust, so no build gains librdkafka.
  `nats` publishes to a subject templated from `{namespace}`, `{queue}`, `{task}` and `{type}`,
  waiting for JetStream acknowledgements by default — with the CloudEvents `id` as
  `Nats-Msg-Id`, so a stream drops retried duplicates — or plain core NATS with
  `mode: "core"`. Each retries or drops by its client's own error taxonomy, names its secrets
  by `*_env`, and sits behind a cargo feature (`events-kafka`, `events-nats`) compiled into
  the published image and every SDK build; a build without one refuses a document naming it.
- **Event egress** (#848). Job lifecycle transitions — `job.enqueued`, `job.started`,
  `job.completed`, `job.failed`, `job.retrying`, `job.dead`, `job.cancelled`, `job.sleeping` —
  can leave the process as a structured-mode CloudEvent, `POST`ed to an HTTP endpoint or
  ...
  Delivery is at-most-once overall and may repeat an event, so consumers dedupe on the
  CloudEvents `id`; emitting never blocks the queue. The HTTP sink goes through push
  dispatch's egress guard (a required allowlist, no proxy, no
  redirects) and signs with `x-flexiq-event-signature` when given a secret; job payloads are
  sent only by a sink that sets `include_payload`. Per-sink counters appear on the server's
  `/metrics` as `flexiq_events_*` and on every worker's event-sink stats. The HTTP sink is
  behind the `events-http` cargo feature, compiled into the published image and SDK builds; the
  Helm chart gains `events.*`. The contract a consumer implements against is
  [`contracts/EVENT_EGRESS_CONTRACT.md`](https://github.com/ByteVeda/flexiq/blob/master/contracts/EVENT_EGRESS_CONTRACT.md);
  the stream is not a complete audit, and its
  [What is not seen](https://github.com/ByteVeda/flexiq/blob/master/contracts/EVENT_EGRESS_CONTRACT.md#what-is-not-seen)
  section lists the transitions that emit nothing.
...
- **Push dispatch** (#843, #844). `FLEXIQ_PUSH_TARGET_URL` adds a second way for the scheduler to
  ...
  request rather than running one continuously. Behind the `http-target` cargo feature, which the
  published `flexiq-server` image compiles in alongside `postgres`, `redis` and `grpc`.
  Every dispatch goes through an egress guard: a deny-by-default allowlist, DNS vetted inside
  resolution so the address dialled is the address checked, loopback/link-local/cloud metadata
  refused whatever the allowlist says, and no redirects. Four outbound auth schemes prove the
  scheduler's identity to the target: a static bearer token, HMAC-SHA256 with replay defence,
  OIDC identity tokens (Cloud Run, Azure IMDS, Azure App Service), and AWS SigV4 (Lambda, API
  Gateway). The wire contract a target implements against is
  [`contracts/PUSH_DISPATCH_CONTRACT.md`](https://github.com/ByteVeda/flexiq/blob/master/contracts/PUSH_DISPATCH_CONTRACT.md).
...
- **A float's width on the wire is pinned, so `auto:` keys no longer diverge over one** (#905).
  ...
  widths decode to the same number, so a narrowing writer interoperated fine and silently stopped
  idempotent enqueues deduping: the `auto:` key is a sha256 over the serialized payload, so two
...
- **A failed Rust task records an empty traceback, not a null one** (#930). `flexiq`'s failure
  formatter wrote `"traceback": null`, a third shape the binding contract does not describe — it
...
- `enqueue_unique_reporting` and `enqueue_unique_batch_reporting` are new required methods. They
  ...
  which is not computable above the backend — the job's id is generated inside the insert, so a
  caller has no candidate to compare against. The existing names stay as one-line wrappers, so a
  backend still has one implementation.
- `reschedule` gained a third parameter, `namespace: Option<&str>`. A step sleep reschedules with
  an id that reached the queue through task code, so the call had to become scoped.

New public fields, which break an exhaustive struct literal:

- `Dispatch.lease`, `DeadJob.origin_job_id`, `DebounceOptions.max_pending`,
  `ExecutorConfig.capabilities`, `ExecutorConfig.step_ack_timeout`, `RemoteConfig.step_workers`,
  `RemoteConfig.step_capacity`.
- `lease` on `ExecutorMessage::{Progress, TaskLog, Success, Failure, Cancelled}` and on
  `SchedulerMessage::Job`.

Newly `#[non_exhaustive]`. The consequence differs by what was marked:

- **Enums** `ExecutorMessage`, `SchedulerMessage`, `JobResult`, `ResultOutcome` — a `match` on
  one now needs a `_` arm.
- **Structs** `AttachedExecutor`, `WorkerInfo`, `WorkerRegistration` — no longer constructible
  with a struct literal outside the crate, and destructuring one needs a trailing `..`.
- **Variant** `ExecutorMessage::Hello` — same two rules as a struct, applied to that variant
  alone; the other variants still construct and destructure as they did.

New variants on enums that were exhaustive in 1.0.0:

- `QueueError::{QueueFull, ClaimLost, StepDiverged, StepSequenceDiverged, StepLimitExceeded,
  StepRefused}`.
- `WorkflowError::InvalidStepMetadata`, split out so a malformed step payload from a caller no
  longer reports as a storage fault.

Changed discriminants, which reach only a numeric cast:
...
- Imports move to `flexiq` / `@byteveda/flexiq` / `org.byteveda.flexiq` / `flexiq_core`, and Java
  types follow: `Taskito` → `FlexiQ`, `TaskitoException` → `FlexiQException`.
- Every `TASKITO_*` environment variable is now `FLEXIQ_*`, with no fallback.
- The default database moves to `~/.flexiq/flexiq.db`; the schema is unchanged.
- Webhook signatures ride in `X-Flexiq-Signature`, the dashboard token in `X-Flexiq-Token`, and
  the session cookie is `flexiq_session`. The spelling is `X-Flexiq-`, not `X-FlexiQ-`, so the name
  survives the title-casing HTTP clients apply to outgoing headers.
- Django commands become `flexiq_worker`, `flexiq_dashboard`, `flexiq_info`.
- The Helm chart is `flexiq-server`.

**Drain your queue before upgrading.** Payload markers changed from `__taskito_*__` to
...
- **Log + cursor pub/sub.** An opt-in `log` topic mode writes one message row per publish (O(1))
  and is consumed by pull — read, process, then ack — with per-message acknowledgement, a
  first-class topic registry, and a managed consumer that tracks cursors for you.
- **Retention, on by default.** Per-table retention windows purge aged job, dead-letter, and
  archived rows. A single reaper and retention cleaner are elected across workers so the sweep
  runs once, and every purge is bounded on all three backends.
- **Overload controls.** An opt-in `max_pending` admission cap rejects enqueues past a ceiling,
  opt-in CoDel load shedding drops work that has dwelled too long, and an opt-in LIFO dispatch
  order serves newest-first.
- **Retry budget.** A per-task token-bucket `retry_budget` caps how fast a task may retry,
  independently of `max_retries`.
- **Native async dispatch.** Coroutine tasks run on a native async executor, with worker drain
  decoupled from Python reference counts.
- **taskito-tui.** A headless terminal UI crate inspects and operates a queue directly against
  its database, with no changes to the core.
- **Native Rust worker.** A native worker with a task registry and dispatcher, plus typed Redis
  and JSON error variants, as part of readying `taskito-core` for standalone use.
- **Cross-SDK parity.** Node and Java gain per-task policy and batching parity; the remaining
  parity gaps are tracked explicitly.
- **Single-source release version.** One version literal in the root manifest drives every SDK,
  checked in CI so a mismatched manifest fails the build.

### Changed

- **Scale to millions of rows.** Keyset pagination and bounded scans replace offset paging and
  full scans across listings and mass mutations.
- **Async backpressure.** In-flight work is bounded to the pool's worker and async concurrency,
  so async pools no longer over-claim.
- **One task lifecycle.** Synchronous and native-async dispatch share a single task-lifecycle
  path in Python.
- **Retention tuning.** PostgreSQL fillfactor and autovacuum are tuned for the retention
  workload, and the Redis archived-retention purge is bounded.

### Fixed

- **Node webhook SSRF guard.** Webhook delivery screens loopback, private, link-local, and
  cloud-metadata targets at registration and re-checks every attempt; redirects are not followed.
- **Node webhook store.** Subscriptions persist in the shared cross-SDK layout, so a hook
  registered by any SDK is visible to all of them.
- **Node log consumers** flush when subscriptions are declared and stop earlier on worker
  shutdown.
- **Negative `result_ttl`** is rejected rather than silently accepted (Python).
- Documentation is served under `/taskito` without a doubled path prefix.
