...
Newly `#[non_exhaustive]` — a `match` now needs a `_` arm, and a literal no longer constructs:

- Enums `ExecutorMessage`, `SchedulerMessage`, `JobResult`, `ResultOutcome`.
- Structs `AttachedExecutor`, `WorkerInfo`, `WorkerRegistration`.
- Variant `ExecutorMessage::Hello`.
