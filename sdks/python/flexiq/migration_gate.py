...
``flexiq migrate`` reaches its queue by importing the application module, which
constructs a :class:`~flexiq.app.Queue` — and a queue that migrates on open
...
#: Set by ``flexiq migrate`` before it imports the application.
