...
import flexiq
from flexiq._flexiq import PyQueue, PyTaskConfig
from flexiq.context import _set_queue_ref
...
from flexiq.log_config import restore_asyncio_pipe_noise, silence_asyncio_pipe_noise
from flexiq.resources.health import HealthChecker
from flexiq.resources.runtime import ResourceRuntime
from flexiq.testing import TestMode
...
"""
    ...
    def event_sink_stats(self) -> list[dict[str, Any]]:
        """Counters for each event sink of the worker running on this queue.

        One dict per sink, in configuration order, with ``name``, ``kind``,
        ``delivered``, ``dropped_buffer_full``, ``dropped_rejected``,
        ``dropped_failed``, ``dropped_shutdown`` and ``queued``. Empty when no
        worker is running or ``event_sinks`` was not set.
        """
    ...
    def test_mode(
        ...
        Returns:
            A :class:`~flexiq.testing.TestMode` context manager.
        """
