...
Set by ``flexiq executor`` before it imports the app, and inherited by the
...
logger = logging.getLogger("flexiq.executor")
...
class DetachedNative:
    ...
    importing itself outside ``flexiq executor``, or a scheduler that
    ...
    def __getattr__(self, name: str) -> NoReturn:
        raise DetachedStorageError(
            ...
            "Use an in-process worker (`flexiq worker`) if this app needs to "
