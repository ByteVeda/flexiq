...
:class:`~flexiq.dashboard.webhook_store.WebhookSubscriptionStore` on
...
if TYPE_CHECKING:
    ...
    from flexiq.dashboard.webhook_store import WebhookSubscription

logger = logging.getLogger("flexiq.webhooks")
...
class WebhookManager:
    ...
    def __init__(self, queue_ref: Queue | None = None, preload: bool = True) -> None:
        # ``queue_ref`` is the parent :class:`flexiq.app.Queue`. Optional
        ...
        with self._lock:
            if self._thread is None or not self._thread.is_alive():
                self._thread = threading.Thread(
                    target=self._deliver_loop, daemon=True, name="flexiq-webhooks"
                )
                self._thread.start()
