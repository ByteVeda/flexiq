...
:class:`flexiq.dashboard.auth.AuthStore`, sessions are enforced. Until the
...
from flexiq.dashboard.errors import _BadRequest, _NotFound
from flexiq.dashboard.handlers.auth import handle_auth_status
from flexiq.dashboard.handlers.oauth import (
    OAuthRedirect,
    handle_providers,
)
from flexiq.dashboard.handlers.oauth import (
...
from flexiq.dashboard.request_context import (
    CSRF_COOKIE,
    SESSION_COOKIE,
    RequestContext,
    build_context,
)
from flexiq.dashboard.routes import (
...
from flexiq.dashboard.static import (
    IMMUTABLE_PREFIX,
    MISSING_ASSETS_HTML,
    StaticAssets,
    _content_type_for,
...
logger = logging.getLogger("flexiq.dashboard")
...
def _build_oauth_flow_from_env(queue: Queue) -> OAuthFlow | None:
    ...
    """
    try:
        from flexiq.dashboard.oauth.config import from_env as oauth_from_env
        from flexiq.dashboard.oauth.flow import OAuthFlow
    ...
    *,
    ...
    class DashboardHandler(BaseHTTPRequestHandler):
        ...
        def _build_context(self) -> RequestContext:
            ...
            session = None
            if cookies_header:
                from flexiq.dashboard.request_context import parse_cookies
