...
/** Command-line interface over a FlexiQ queue. */
@Command(
        name = "flexiq",
...
public final class Cli {
    ...
    FlexiQ open(boolean autoMigrate) {
        ...
        FlexiQ.Builder builder = FlexiQ.builder().backend(backend).autoMigrate(autoMigrate);
        if (url != null) {
            builder.url(url);
        }
        return builder.open();
    ...
    static final class Migrate implements Callable<Integer> {
        ...
        public Integer call() {
            ...
            try (FlexiQ queue = parent.open(false)) {
    ...
    static final class Executor implements Callable<Integer> {
        ...
        public Integer call() throws Exception {
            ...
            org.byteveda.flexiq.worker.Executor.Builder builder = org.byteveda.flexiq.worker.Executor.builder()
            ...
            try (org.byteveda.flexiq.worker.Executor executor = builder.start()) {
                System.out.printf(
                        "flexiq executor %s attached to %s at %s (%d slot(s), %d task(s)) — Ctrl-C to stop%n",
                        executor.executorId(),
            ...
            System.out.println("flexiq executor detached");
            return 0;
        }
