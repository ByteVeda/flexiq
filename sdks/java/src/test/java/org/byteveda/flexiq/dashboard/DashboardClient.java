...
final class DashboardClient {
    ...
    static AuthStore store(FlexiQ queue) {
        return new AuthStore(SettingsAccess.of(queue));
    }

    static Session seedAdmin(FlexiQ queue) {
        return seedUser(queue, "admin", Role.ADMIN);
    }

    static Session seedUser(FlexiQ queue, String username, Role role) {
